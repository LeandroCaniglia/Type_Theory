...
\begin{defn}
    ...
    \[
        \tau\colon\prod_{x\colon A}\fun{ap}_f(\vartheta(x))\eq{f(x)\eq Bf(x)}\eta_{f(x)}.
    \]
\end{defn}
...
\begin{proof}
    ...
    \begin{equation}\label{eq:coherence-qinv-2}
    ...
    \begin{equation}\label{tikz:coherence-3}
        ...
        \end{tikzcd}
    \end{equation}
    i.e., $\vartheta_{g(f(x))}\eq A\fun{ap}_{g\circ f}(\vartheta_x)$. 
    
    Therefore, we can walk \eqref{tikz:coherence-2} as follows:
    \begin{align*}
...
\section{Equality in \textSigma-Types}
...
\section{Equality Identity Types}
...
\begin{proof}
    ...
    \begin{alignat*}2
        \eta\colon&\prod_{b\colon B}f\circ f^{-1}(b)\eq Bb,
        &\qquad\vartheta\colon&\prod_{a\colon A}f^{-1}\circ f(a)\eq Aa.
    \end{alignat*}
    ...
    and we can define
    \[
        h(q)\defeq\vartheta(a)^{-1}\ct\fun{ap}_{f^{-1}}(q)\ct\vartheta(a').
    \]
    Then
    \begin{align*}
        \fun{ap}_f\circ h(q)
            &\equiv\fun{ap}_f\bigl(\vartheta(a)^{-1}
                \ct\fun{ap}_{f^{-1}}(q)
                \ct\vartheta(a')\bigr)\\
            &\eq B\fun{ap}_f(\vartheta(a))^{-1}
                \ct\fun{ap}_{f\circ f^{-1}}(q)
                \ct\fun{ap}_f(\vartheta(a')).
    \end{align*}
    Applying \eqref{eq:nat-transformation} to $\eta\colon f\circ f^{-1}\sim\id$, we obtain
    \[
        \begin{tikzcd}[column sep=huge,row sep=large]
            f\circ f^{-1}(f(a))
    ...
    \fun{ap}_f(h(q))
        ...
        \ct\fun{ap}_f(\vartheta(a'))
\end{align*}
To reduce this further, recall the coherence conditions of an equivalence (specifically, a half-adjoint equivalence). Consider how\/ $\fun{ap}_f(\vartheta(x))$ and\/ $\eta_{f(x)}$ are related.
\end{proof}