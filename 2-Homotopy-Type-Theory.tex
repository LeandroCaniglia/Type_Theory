...
\begin{proof}
    ...
    \begin{alignat*}4
        ...
        \Phi(w)&\colon B(y,p_*(u)),
            &\quad\fun{Tr}(p,f,u)&\colon B(y,p(u)).
    \end{alignat*}
    Then all expressions are coorectly typed.
    ...
    \begin{enumerate}[-]
        \item $p$ and $p^{-1}$ reduce definitionally $\fun{refl}_x$.
...
\begin{thm}
    ...
    \[
        i(p,u)\ct j(p,u)\ct\hat q(u),
    \]
    where\/ $i(p,u)$ and $j(p,u)$ are the paths given by\/ {\upshape Lemma~\ref{lem:dependent-i-j-path}}.
\end{thm}
...
\begin{proof}
    ...
    \begin{enumerate}[-]
        \item $\fun L(x,\fun{refl}_x,g)\equiv f\eq{}g$. Since
        $\fun{Tr}(\fun{refl}_x,f,u)\equiv f(u)$ and $(\fun{refl}_x)_*(u)\equiv u$, we have $\fun R(x,\fun{refl}_x,g)\equiv f\sim g$.

        \item $i(\fun{refl}_x,u)$ and $j(\fun{refl}_x,u)$ reduce to $\fun{refl}_{f(u)}$.

        \item Take $e\defeq\fun{happly}$. The type $\fun E(x,\fun{refl}_x,g,\fun{happly})$ requires us to verify that for every\/ $q\colon f\eq{}g$ and\/ $u\colon A(x)$,
        \[
            \fun{happly}(q,u)
            \eq{B(x,u)}
            \fun{refl}_{f(u)}
