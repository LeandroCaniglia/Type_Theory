...
\begin{lem}{\upshape[Path Lifting Property]}
    Let\/ $P\colon A\to\univ$ be a type family over\/~$A$. Given\/ $a\colon A$ and\/ $u\colon P(a)$, for any path\/ $p\colon a\eq A b$ with\/ $b\colon A$, there exists a path
    \[
    ...
    \end{enumerate}
    where\/ $\pi_1\colon \sum_{x\colon A}P(x) \to A$ denotes the first projection.\footnotemark
\end{lem}
\footnotetext{Since\/ $\pi_1$ maps the total space to the base type\/ $A$, the functorial action\/ $\fun{ap}_{\pi_1}$ projects paths in\/ $\sum_{x\colon A}P(x)$ to paths in\/ $A$.}
