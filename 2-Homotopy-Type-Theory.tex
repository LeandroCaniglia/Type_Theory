...
\begin{proof}
    As we saw in Remark~\ref{rem:fibration} and Theorem~\ref{thm:path-lift}, and Notation~\ref{ntn:p_*}, since
    \[
        \fun{ap}_{\pi_1}(p)\colon\pi_1(\omega)\eq A\pi_1(\omega'),
    \]
    we have
    \[
        \fun{ap}_{\pi_1}(p)_*(\pi_2(\omega))\eq{P(\pi_1(p))}\pi_2(\omega').
    \]
    Therefore, we have to prove that the function $f$ defined by
    \[
        f\defeq\lambda p.\,\fun{ap}_{\pi_1}(p)_*
    \]
    satisfies $\fun{isequiv}(f)$.

    To define a function in the other direction, we use induction on the $\Sigma$-type. For we set
    \[
        B(\omega')\defeq\omega\eq E\omega',
    \]
    and introduce the motive
    \begin{align*}
        C&\colon\sum_{\omega'\colon E}B(\omega')\to\univ\\
        C((\omega',q))&\defeq\fun{ap}_{\pi_1}(q)_*
            (\pi_2(\omega))\eq{P(\pi_2(\omega'))}\pi_2(\omega').
    \end{align*}
    We specify
    \begin{align*}
        g&\colon\prod_{\omega'\colon E}\;
        \prod_{q\colon\omega\eq E\omega'}C((\omega',q))\\
    g(\omega',q)&\defeq
        \fun{lift}(\pi_2(\omega),\fun{ap}_{\pi_1}(q)).
    \end{align*}
    Since $q\colon\omega\eq E\omega'$, we have $\pi_2(\omega)\colon P(\pi_1(\omega))$ and $\fun{ap}_{\pi_1}(q)\colon\pi_1(\omega)\eq A\pi_1(\omega')$. By Theorem~\ref{thm:path-lift}, we can compute $\fun{lift}$ on the pair
    \[
        \bigl(\pi_2(\omega),\fun{ap}_{\pi_1}(q)\bigr).
    \]
    Moreover, by the same theorem, $g(\omega',q)\colon C(\omega',q)$, so $g$ is well defined. 
