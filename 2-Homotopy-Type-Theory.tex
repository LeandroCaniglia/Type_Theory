...
\begin{prop}\label{prop:pair-properties}
    ...
    \begin{alignat*}{3}
        ...
        \item $\fun{pair}^=(p,q)\ct\fun{pair}^=(p',q')\eq{A\times B}\fun{pair}^=(p\ct p', q\ct q')$
    \end{enumerate}
\end{prop}
...
\begin{proof}${}$
    \begin{enumerate}[i), font=\upshape]
        ...
        \[
            \fun{pair}^=(\fun{refl}_a,\fun{refl}_b)\equiv\fun{refl}_{(a,b)},
        \]
...
\begin{align*}
    ...
    \fun{Code}(\omega,\omega') &\defeq \sum_{q\colon\pi_1(\omega)\eq{A}\pi_1(\omega')}
        \left( \fun{transport}^P(q,\pi_2(\omega)) \eq{P(\pi_1(\omega'))} \pi_2(\omega') \right).
\end{align*}
...
\]
For the second component of $c$, since $\pi_2$ is a dependent function,\footnote{See \nref{lpar:Sigma-induction}, equation \eqref{eq:Sigma-pi-2}.} we have to use the dependent application map $\fun{apd}$.\footnote{See Lemma~\ref{lem:dependent-map}.} Thus, $\fun{refl}_{\pi_1(\omega)}\equiv\fun{apd}_{\pi_2}(\omega)$ and $c$ can be written as
\[
    c\equiv
...
\begin{thm}
    ...
    \[
        \fun{transport}^R(p)(u,z)\eq{R(y)} \left(
...
\section{Function Extension Axiom}