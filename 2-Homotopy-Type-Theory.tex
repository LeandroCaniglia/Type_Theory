...
\begin{defn}
    ...
    \begin{enumerate}[a),font=\upshape]
        \item Given a path $p\colon x\eq Ay$ and a $2$-path $\beta\colon q\eq{y\eq Az}r$ between paths $q,r\colon y\eq Az$,
        \[
        ...
        \[
            p\cdot\alpha\colon(p\ct q)\eq{x\eq Az}(p\ct r)
        \]
        by induction on $\beta$. If $r\equiv q$ and $\beta\equiv\fun{refl}_q$, then $p\cdot\fun{refl}_q\defeq\fun{refl}_{p\ct q}$.
...
\begin{defn}
    ...
    \]
    defined by\footnote{See Remark \ref{rem:Omega-abuse}.}
    \[
...
\begin{defn}
    ...
    \[
        ...
        \end{tikzcd}
    \]
    The \textsl{horizontal composition} of $\alpha$ and $\beta$ is the homotopy
    \[
        \alpha\hct\beta\colon(p\ct r)\eq{a\eq Ac}(q\ct s)
    \]
    defined by path induction on $\alpha$ and $\beta$. Explicitly, if $\alpha \equiv \fun{refl}_p$ and $\beta \equiv \fun{refl}_r$, we define
    \[
...
\begin{rem}
    The horizontal composition can also be computed in terms of whiskering (up to homotopy) as:
    \[
        \alpha\hct\beta\eq{(p\ct r)\eq{a\eq Ac}(q\ct s)}(\alpha\cdot r)\ct(q\cdot\beta).
    \]
\end{rem}

\begin{lem} {\upshape[Interchange Law]}
    Let\/ $A$ be a type and\/ $\alpha, \beta, \gamma, \delta$ be homotopies in\/ $A$ arranged in a commutative square (meaning their boundaries match appropriately). Then, the horizontal composition\/ $\hct$ and the vertical composition\/ $\ct$ satisfy:
    \[
