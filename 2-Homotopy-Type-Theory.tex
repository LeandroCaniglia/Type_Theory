...
\begin{thm} {\upshape[Transitivity on pairs]}\label{thm:sigma-transitivity}
    Let\/ $P\colon A\to\univ$ be a type family and $E\defeq\sum_{x\colon A}P(x)$ its total space.
    Given elements\/ $\omega,\omega',\omega''\colon E$ and paths
    \begin{itemize}
        \item $\rho\defeq\fun{pair}^=(p,q)\colon\omega\eq E\omega'$, with\/ $p\colon x\eq Ax'$ and\/ $q\colon p_*(u)\eq{P(x')}v$

        \item $\rho'\defeq\fun{pair}^=(p',q')\colon\omega'\eq E\omega''$, with\/ $p'\colon x'\eq A x''$ and\/ $q'\colon p'_*(v)\eq{P(x'')}w$,
    \end{itemize}
    the concatenation\/ $\rho \ct \rho'$ corresponds to the pair\/ $(p \ct p', \gamma)$, where\/ $\gamma$ is the composite path:
    \[
        (p\ct p')_*(u)\xrightarrow{\ \zeta\ } p'_*(p_*(u)) \xrightarrow{\fun{ap}_{p'_*}(q)} p'_*(v) \xrightarrow{\ q'\ }w.
    \]
    Here, $\zeta$ is the transport coherence path\/ $(p\ct p')_*(u) \eq{} p'_*(p_*(u))$.
    In terms of the constructor:
    \[
        \fun{pair}^=(p,q)\ct\fun{pair}^=(p',q')
        \eq E
        \fun{pair}^=(p\ct p',\zeta
            \ct\fun{ap}_{p'_*}(q)\ct q').
    \]
\end{thm}

\begin{proof}
    To prove the equality, we proceed by path induction on $p$ and $p'$. Consequently, it suffices to assume $x', x''\equiv x$ and $p,p'\equiv\fun{refl}_x$.
    
    Then the transport maps $p_*$ and $p'_*$ reduce to the identity, and the coherence $\zeta$ becomes $\fun{refl}_u$.
    
    The goal then becomes
    \[
        \fun{pair}^=(\fun{refl}_x,q)
        \ct\fun{pair}^=(\fun{refl}_x,q')
        \eq E
        \fun{pair}^=(\fun{refl}_x,q\ct q').
    \]
    Recalling that $\fun{pair}^=(\fun{refl}_x, \gamma) \equiv \fun{ap}_{\fun{in}_x}(\gamma)$,\footnote{Definition~\ref{defn:Sigma-pair} \&~Theorem~\ref{thm:path-lift}.} this equality becomes:
    \[
        \fun{ap}_{\fun{in}_x}(q)\ct\fun{ap}_{\fun{in}_x}(q')
        \eq E
        \fun{ap}_{\fun{in}_x}(q\ct q'),
    \]
    which holds by the functoriality of $\fun{ap}$.\footnote{Remark~\ref{rem:types-as-categories}.}
    \[
    \begin{tikzpicture}[
        >=stealth, 
        scale=1.0,
...
\begin{proof}
    Define the forward map as
    \begin{align*}
        f&\colon\prod_{x,y\colon\fun1}(x\eq{\fun1}y) \to\fun1\\
        f(x,y,p)&\defeq\star.
    \end{align*}
    To define the inverse map $g$, we proceed by induction on $x$.
    It suffices to construct $g(\star)$.
    The goal becomes a term of type
    \[
        \prod_{y\colon\fun1}\fun1 \to (\star\eq{\fun1}y).
    \]
    Induction on $y$ further reduces the goal to $\fun1 \to (\star\eq{\fun1}\star)$, which is inhabited by $\fun{refl}_\star$.
    
    Thus, we have defined $g\colon\prod_{x,y\colon\fun1}\fun1\to(x\eq{\fun1}y)$, such that $g(\star,\star,\star) \equiv \fun{refl}_\star$.
    ...
    \begin{enumerate}[a), font=\upshape]
        ...
        By the induction principle of $\fun1$, we may set $x,y,u\equiv\star$, which satisfies the goal because $f$ maps everything to $\star$.

        \item $g\circ f \sim \id_{\prod_{x,y\colon\fun1}(x\eq{\fun1}y)}$.
        
        We must show $g(x,y,f(x,y,p))\eq{x\eq{\fun1}y}p$ for all $x,y\colon\fun1$ and $p\colon x\eq{\fun1}y$.
        By path induction on $p$, we can set $y\equiv x$ and $p\equiv\fun{refl}_x$.
        Then the goal becomes $g(x,x,f(x,x,\fun{refl}_x))\eq{x\eq{\fun1}x} \fun{refl}_x$, which reduces to $g(x,x,\star) \eq{x\eq{\fun1}x}\fun{refl}_x$.
        Induction on $x$ leaves us with $g(\star,\star,\star)\eq{\star\eq{\fun1}\star}\fun{refl}_\star$, which holds by the definition of $g$.
    \end{enumerate}
\end{proof}
