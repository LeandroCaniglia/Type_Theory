...
\begin{proof}${}$
    \begin{description}[font=\normalfont\itshape]
        ...
        \item[Transitivity.]
        We prove $\fun{encode}(p \ct q) \eq{} \fun{encode}(p) \ect \fun{encode}(q)$ by path induction on $p$.
        ...
        \end{align*}
        By the hypothesis $c\ect d\equiv d$, the RHS reduces to $\fun{encode}(q)$.
...
\begin{thm}\label{thm:composite-transport}
    Let\/ $A\colon X\to\univ$ be a type family and let\/ $\Sigma_XA\defeq\sum_{x\colon X}A(x)$ its total space. Given a second type family\/ $B\colon \Sigma_XA\to\univ$,
    define the type family\/ $\Sigma_AB\colon X\to\univ$ by\/ $\Sigma_AB(x)\defeq\sum_{u\colon A(x)}B(x,u)$. Let\/ $\Sigma_{\Sigma_XA\,}B\defeq\sum_{\omega\colon \Sigma_XA}B(\omega)$ be the total space of the composite family. This forms a tower of fibrations:
    \[
...
\[
    % Left: Path in function space
    \begin{tikzpicture}[
        baseline=(current bounding box.center),
        font=\small]
        \node (A) at (0,2) {$\displaystyle{
            \Gamma\equiv\prod_{x\colon A}B(x)}$};
        \node (f) at (-1,0.5) {$f$};
        \node (g) at (1,0.5) {$g$};
        \draw[->, thick] (f) to[bend left=20] node[above] {$p$} (g);
        \node at (0, -0.7) {\text{Path in function space}};
    \end{tikzpicture}
    \hspace{2cm}
    % Right: Homotopy as a mapping
    \begin{tikzpicture}[baseline=(current bounding box.center),
        ...
        \draw[|->] (x) -- node[above, sloped] {} (fx);
        \draw[|->] (x) -- node[below, sloped] {} (gx);
        \draw[double, double distance=2pt] (fx) -- node[right] {$\fun{happly}(p)(x)$} (gx);
        
        \node at (1.5, -1.0) {\text{Pointwise function homotopy}};
    \end{tikzpicture}
\]
...
\begin{proof}
    ...
    \begin{description}[font=\normalfont\scshape]
        ...
        \]
        In our case, $\omega\defeq(y,v)$, $\omega'\defeq(x,w)$ and $q\defeq p^{-1}$. Thus, we can apply $\fun{pair}^=$ to the dependent pair $(q,r)$, provided that $r\colon q_*(v)\eq{A(x)}w$. Since $q_*(v)\defeq(p^{-1})_*(v)\defeq w$, we deduce that $(p^{-1},(p^{-1})_*(v))\colon\dom(\fun{pair}^=)$. Moreover,
        \[
            \delta\defeq
                \fun{pair}^=(p^{-1},(p^{-1})_*(v))\colon(y,v)
                \eq{\Sigma_XA}(x,(p^{-1})_*(v)).
        \]
        Therefore,
        \[
        ...
        \[
            ...
            B(x,w)\to B(y,v).
        \]
        Since $f(w)\colon B(x,w)$, 
        \[
            \rhs\equiv\fun{transport}^{\hat B}(\delta^{-1},f(w))
                \colon B(y,v).
        \]
        \item[Inhabitation:] By path induction on $p$, we reduce ourselves to the case $y\equiv x$ and $p\equiv\fun{refl}_x$. We obtain
        \begin{align*}
            \lhs&\equiv
...
\begin{thm}\label{thm:dependent-function-equality-equivalence}
    ...
    \[
        ...
        \end{tikzcd}
    \]
    where $\Phi$ is defined in\/ {\upshape Lemma~\ref{lem:dependent-i-j-path}}.
\end{thm}
...
\begin{proof}${}$
    \begin{description}[font=\normalfont\scshape]
        ...
        For the \lhs, observe that $p_*(f)$ and $g$ both inhabit the function type $\Pi_AB(y)$; thus, the identity type $p_*(f)=g$ is well-formed.
...
\begin{description}[font=\normalfont\scshape]
    ...
    \item[Base code.]
    We specify the base code \/$c \colon \fun{Code}(A,A)$ as \/$c \defeq \id_A$. This is an abuse of notation for the canonical equivalence
    \[
        (\id_A,(\id_A,\fun{refl},\id_A,\fun{refl}))\colon A\simeq A
    \]
    [cf.~Proposition~\ref{prop:equivalence-is-an-equivalence-relation}].
...
\begin{proof}
    ...
    \begin{align*}
        g&\colon(x\eq Az)\to(y\eq Az)\\
        g(r)&\defeq p^{-1}\ct r.
    \end{align*}
    Then, $g\equiv\fun{transport}^{\fun{Eq}_z}(p^{-1})$. Moreover,
    \begin{align*}
        g(t(q)) &\equiv g(p\ct q)\\
            &\equiv p^{-1}\ct(p\ct q)\\
...
\begin{rem}\label{rem:Omega-abuse}
    ...
    Note however that the reason to define $\Omega\colon\puniv\to\puniv$ and not from $\puniv$ to $\univ$, is to enable the definition of $\Omega^n$ for $n=0,1,\dots$.
\end{rem}
...
\begin{defn}
    ...
    \]
    defined by\footnote{As noted in Remark \ref{rem:Omega-abuse}, by abusing notation $\Omega(A,a)$ sometimes refers to $a\eq Aa$ rather than to the pointed type.}
    \[
