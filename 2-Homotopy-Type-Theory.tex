...
\begin{prop}
    Let\/ $A$ and\/ $B$ be types. Then the maps
    \begin{align*}
        f&\colon(x\eq{A\times B}y)
            \to
            (\pi_1(x)\eq A\pi_1(y))\times(\pi_2(x)\eq B\pi_2(y))\\
        &\quad p\mapsto(\fun{ap}_{\pi_1}(p),\fun{ap}_{\pi_2}(p)),
            \\[1ex]
        g&\colon(\pi_1(x)\eq A\pi_1(y))\times(\pi_2(x)
            \eq B\pi_2(y))
            \to(x\eq{A\times B}y)\\
        &\quad(u,v)\mapsto\fun{uniq}_{A\times B}(x)^{-1}
            \ct\fun{pair}^=(u,v)
            \ct\fun{uniq}_{A\times B}(y)
    \end{align*}
    satisfy $g\circ f\sim\id$.
\end{prop}

\begin{proof}
    By Lemma~\ref{lem:pre-composition-with-rho}, it suffices to verify that the functions agree upon pre-composition with the reflexivity section\/ $\rho_{A\times B}$. That is, for every\/ $z\colon A\times B$, we must show:
    \[
        (g\circ f)(\fun{refl}_z) \eq{} \fun{refl}_z.
    \]
    First, we compute the action of\/ $f$ on the reflexivity path:
    \[
        f(\fun{refl}_z)
        \equiv(\fun{ap}_{\pi_1}(\fun{refl}_z),
            \fun{ap}_{\pi_2}(\fun{refl}_z))
        \equiv(\fun{refl}_{\pi_1(z)},\fun{refl}_{\pi_2(z)}).
    \]
    Next, we apply\/ $g$ to this pair. Note that since the starting path was a loop on\/ $z$, we have\/ $x \equiv y \equiv z$. The definition of\/ $g$ becomes a conjugation:
    \[
        g(\fun{refl}_{\pi_1(z)},\fun{refl}_{\pi_2(z)})
        \equiv\fun{uniq}_{A\times B}(z)^{-1}
            \ct\fun{pair}^=(\fun{refl}_{\pi_1(z)},
                \fun{refl}_{\pi_2(z)})\ct\fun{uniq}_{A\times B}(z).
    \]
    The term\/ $\fun{pair}^=$ applied to reflexivities computes definitionally to the reflexivity of the pair:
    \[
        \fun{pair}^=(\fun{refl}_{\pi_1(z)}, \fun{refl}_{\pi_2(z)}) \equiv \fun{refl}_{(\pi_1(z), \pi_2(z))}.
    \]
    Substituting this back, the expression simplifies via the unit laws for path concatenation:
    \begin{align*}
        \fun{uniq}_{A\times B}(z)^{-1}
            \ct\fun{refl}_{(\pi_1(z),
                \pi_2(z))}\ct\fun{uniq}_{A\times B}(z)
        &\equiv\fun{uniq}_{A\times B}(z)^{-1}
            \ct\fun{uniq}_{A\times B}(z)\\
        &\eq{z\eq{A\times B}z}\fun{refl}_z.
    \end{align*}
    Thus, the equality holds, completing the proof.
\end{proof}
...
\begin{description}[font=\normalfont\scshape]
    ...
    \begin{thm}
        Given\/ $A,B\colon\univ$, there is a function
    ...
    \begin{defn}
        By the theorem, we can define $\fun{encode}\defeq\fun{idtoeqv}$.
    \end{defn}
...
\begin{prop}
    ...
    \begin{align*}
        ...
        t(q)&\defeq p\ct q
    \end{align*}
    is an equivalence definitionally equal to\/ $\fun{transport}^{\fun{Eq}_z}(p)$, where\/ $\fun{Eq}_z \colon A \to \univ$ is the type family\/ $\fun{Eq}_z(w)\defeq w\eq Az$.
\end{prop}

\begin{proof}
    The equality between $t$ and $\fun{transport}^{\fun{Eq}_z}$ is clear. To see that it has a quasi-inverse, consider
    \begin{align*}
        s&\colon(x\eq Az)\to(y\eq Az)\\
        s(r)&\defeq p^{-1}\ct r.
    \end{align*}
    Then, $s\equiv\fun{transport}^{\fun{Eq}_z}(p^{-1})$. Moreover,
    \begin{align*}
