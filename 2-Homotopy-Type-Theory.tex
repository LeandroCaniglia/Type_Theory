...
\begin{rem}
    ...
    Given $a,b\colon A$, the type family $P$ structurally relates the fibers $P(a)$ and $P(b)$. Although a point $u\colon P(a)$ cannot be directly compared with $v\colon P(b)$ due to type mismatch, it is possible to establish a connection between the pairs $(a,u)$ and $(b,v)$ within the $\Sigma$-type. Specifically, if there exists a path $p\colon a\eq A b$, we can map $u$ along $p$ via transport to obtain an element $\fun{transport}^P(p,y)\colon P(b)$. Since this transported term inhabits the same fiber as $v$, the comparison becomes well-defined. More precisely:
    
\end{rem}

\begin{lem}{\upshape[Path Lifting Property]}
    Let\/ $P\colon A\to\univ$ be a type family over\/~$A$. Given\/ $a\colon A$ and\/ $u\colon P(a)$, for any path\/ $p\colon a\eq Ab$ with $b\colon A$, there exists a path
    \[
        \fun{lift}(u,p)\colon(a,u)\eq{\sum_{x\colon A}P(x)}
            (b,\fun{transport}^P(p,u))
    \]
    such that\/ $\fun{lift}(u,\fun{refl}_a) \equiv \fun{refl}_{(a,u)}$ and
    \[
        \fun{ap}_{\pi_1}(\fun{lift}(u,p))\eq{a\eq Ab}p,
    \]
    where\/ $\pi_1\colon \sum_{x\colon A}P(x) \to A$ denotes the first projection.\footnote{Since $\pi_1$ is a function from the total space to $A$, $\fun{ap}_{\pi_1}$ maps paths in the total space to paths in~$A$.}
\end{lem}
