...
\newcommand{\ev}[1]{{\op ev}_{#1}}
\newcommand{\im}[1]{\op{im}#1}
\newcommand{\fun}[1]{\mathsf{#1}}
\newcommand{\id}{\fun{id}}
\newcommand{\nset}[2][1]{\llbracket#1,#2\rrbracket}
\newcommand{\dom}{\mathrm{dom}}
\newcommand{\epi}{{\mbf{\op{epi}}}}
\newcommand{\cl}{\mathrm{cl}}
\newcommand{\hypo}{{\mbf{\op{hypo}}}}
\newcommand{\prob}{{\op prob}}
...
\newcommand{\lqed}{\textcolor[gray]{0.5}{\small$\blacksquare$}}
\newcommand{\localqed}{\hfill\lqed}
...
\newcommand{\Sym}[1]{{\op Sym}#1}
\newcommand{\Alt}[1]{{\op Alt}#1}
%\newcommand{\Aut}[1]{{\op GL}#1}
\DeclareMathOperator{\Aut}{Aut}
\newcommand{\Elt}{\mathrm{Elt}}
\newcommand{\Hom}{{\op Hom}}
\newcommand{\End}[1]{{\op{End}}#1}
\newcommand{\Inn}[1]{{\op Inn}#1}
\newcommand{\Syl}[1]{{\op Syl}#1}
\newcommand{\Hall}[1]{{\op Hall}#1}
\newcommand{\Core}{{\op Core}}
\newcommand{\normal}{\lhd}
...
% Draw the TikZ picture exactly ONCE in math mode and store it
\sbox{\eqbox}{%
  $\vcenter{\hbox{\tikz{
    \draw[line width=0.4pt] (0,0) circle (0.8ex);
    \draw[-{Latex[length=0.6ex, width=0.5ex]}, line width=0.4pt] (-0.8ex,0) -- (0.8ex,0);
  }}}$%
}

% Reuse the pre-drawn box in \mathchoice
\newcommand{\oeq}[1]{%
  \mathrel{\mathchoice
    {\usebox{\eqbox}}% \displaystyle
    {\usebox{\eqbox}}% \textstyle
    {\scalebox{0.7}{\usebox{\eqbox}}}% \scriptstyle
    {\scalebox{0.5}{\usebox{\eqbox}}}% \scriptscriptstyle
  }_{#1}%
}

\newcommand{\modeq}[1]{\quad (\text{in } #1)} % Usage: p=q\modeq{x\eq{A}y}
\newcommand{\ct}{\mathbin{\boldsymbol{\cdot}}}
\newcommand{\ect}{\mathbin\ast}
\newcommand{\hct}{\mathbin{\vcenter{\hbox{$\scriptstyle\bullet$}}}}
\newcommand{\fstar}{{\fun{\star}}}
\newcommand{\lwct}{\triangleleft}
\newcommand{\rwct}{\triangleright}
...
\newcommand{\puniv}{{\univ_\sbull}}

\newcommand{\explain}[2][]{\underbracket[0.5pt][2pt]{#2}_{#1}}

\newcommand{\tac}{\fun{ac}_0}

\newcommand{\dummy}{{\mathord-}}

\newcommand{\dep}[1]{\langle#1\rangle}
\newcommand{\bigdep}[1]{\bigl\langle#1\bigr\rangle}
\newcommand{\Bigdep}[1]{\Bigl\langle#1\Bigr\rangle}

\newcommand{\yoneda}{\mbf{\mathit h}}

\newcommand{\gref}[1]{{\hypersetup{linkcolor=gray}\textcolor{gray}{#1}}}


\usepackage{imakeidx}

% Define the command that takes one argument and completely ignores it
\newcommand{\emptyindexheading}[1]{}

% Set layout configurations
\indexsetup{level=\emptyindexheading, noclearpage}

% Declare both indexes
\makeindex[name=fe, columns=2, options=-s custom.ist]
\makeindex[name=we, columns=2, options=-s custom.ist]
\makeindex[name=ua, columns=2, options=-s custom.ist]

\newcounter{axiomsortcount}

\makeatletter
\def\idxname@thm{Theorem}
\def\idxname@lem{Lemma}
\def\idxname@prop{Proposition}
\def\idxname@cor{Corollary}
\def\idxname@exr{Exercise}
\def\idxname@rem{Remark}

\newcommand{\usesaxiom}[1]{%
  \ifcsname idxname@\@currenvir\endcsname
    \edef\indexed@name{\csname idxname@\@currenvir\endcsname}%
  \else
    \def\indexed@name{Theorem}%
  \fi% <--- Blank line removed here
  \stepcounter{axiomsortcount}%
  \edef\sortkey{%
    \ifnum\c@axiomsortcount<1000 0\fi
    \ifnum\c@axiomsortcount<100 0\fi
    \ifnum\c@axiomsortcount<10 0\fi
    \the\c@axiomsortcount
  }%
  \edef\doindex{\noexpand\index[#1]{\sortkey @\indexed@name\space\csname the\@currenvir\endcsname|hyperpage}}%
  \doindex
}
\makeatother

% Generic macro for the visual marker and spacing
% #1 = Index internal name (e.g., fe)
% #2 = Printed text (e.g., fe)
\newcommand{\axiommark}[2]{%
  \hspace*{-0.15cm}%
  \smash{\raisebox{0.6ex}{\normalfont\upshape\textsc{\scriptsize #2}}}%
  \hspace*{0.15cm}%
  \usesaxiom{#1}%
}

% Define your context-aware shortcuts
\newcommand{\fe}{\axiommark{fe}{fe}}
\newcommand{\we}{\axiommark{we}{we}}
\newcommand{\ua}{\axiommark{ua}{ua}}

\newcommand{\FE}{\text{\upshape\textsc{fe}}}
\newcommand{\WE}{\text{\upshape\textsc{we}}}
\newcommand{\UA}{\text{\upshape\textsc{ua}}}
\newcommand{\NE}{\text{\upshape\textsc{ne}}}
...
\newcommand{\Lem}{\text{\upshape\textsc{lem}}}
