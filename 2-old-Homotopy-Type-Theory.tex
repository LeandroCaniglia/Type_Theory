...
\begin{defn}
    ...
    \begin{align*}
        \Omega&\colon\univ_\bullet\to\univ_\bullet\\
...
\begin{rem}\label{rem:Omega-abuse}
    ...
    Note however that the reason to define $\Omega\colon\univ_\bullet\to\univ_\bullet$ and not from $\univ_\bullet$ to $\univ$, is to enable the definition of $\Omega^n$ for $n=0,1,\dots$.
\end{rem}
...
\begin{prop} {\upshape[$\fun{ap}$ as functor]}\label{prop:ap-as-functor}
...
\begin{defn}
    Let $A$ be a type and let $a,b,c\colon A$ be points. Consider paths $p,q\colon a\eq Ab$ and $r,s \colon b\eq Ac$, along with homotopies $\alpha\colon p\eq{a\eq Ab}q$ and $\beta\colon r\eq{b\eq Ac}s$, as depicted below:
    \[
...
\begin{prop}
    Let\/ $f$ and\/ $g$ be the functions defined previously between the path space of the product and the product of path spaces. Then\/ $g\circ f\sim\id$.
\end{prop}

\begin{proof}
    By the previous Lemma, to prove that $g\circ f$ is homotopic to the identity function on the path space, it suffices to verify that they agree upon pre-composition with $\fun\rho$. That is, we must check that for every $x\colon A\times B$:
    \[
        (g\circ f)(\fun{refl}_x)\eq{}\id(\fun{refl}_x).
    \]
    By the definition of $f$, we have $f(\fun{refl}_x)\equiv(\fun{refl}_{\fun\pi_1(x)}, \fun{refl}_{\fun\pi_2(x)})$. Applying $g$ to this canonical pair yields the reflexivity path on the pair of projections:
    \[
        (g\circ f)(\fun{refl}_x)
            \equiv\fun{refl}_{(\fun\pi_1(x),\fun\pi_2(x))}.
    \]
    We must compare this to the right-hand side, which is simply $\fun{refl}_x$. We invoke the uniqueness homotopy $\fun{uniq}_{ \times B}(x)\colon(\fun\pi_1(x),\fun\pi_2(x))\eq{}x$. Applying the congruence of the reflexivity term (whiskering by $\fun{refl}$), we obtain the path:
    \[
        \fun{refl}\cdot\fun{uniq}_{A\times B}(x)\colon \fun{refl}_{(\fun\pi_1(x),\fun\pi_2(x))} \eq{} \fun{refl}_x.
    \]
    This provides the required homotopy for every $x$. Thus, by lemma, $g\circ f\sim\id$.
\end{proof}
...
\begin{lem}\label{lem:equivalence-composition}
    ...
    \begin{equation}\label{eq:equivalence-composition}
        \begin{aligned}
            \eta&\defeq
                (g\cdot\eta_1\cdot h_2)\ct\eta_2
                    \colon g\circ f\circ h_1\circ h_2\sim\id_C\\
            \vartheta&\defeq
                (k_1\cdot\vartheta_2 \cdot f)\ct\vartheta_1
...
\begin{prop}\label{prop:qinv-composition}
    ...
    \begin{enumerate}[a),font=\upshape]
        ...
        \begin{align*}
            \eta&\defeq
                (g\cdot\eta_f\cdot g^{-1})\ct\eta_g
                    \colon g\circ f\circ f^{-1}\circ g^{-1}\sim\id_C\\
            \vartheta&\defeq
                (f^{-1}\cdot\vartheta_g\cdot f)\ct\vartheta_f
...
\begin{cor}\label{cor:qinv-composition-with-id}
    ...
    \begin{enumerate}[a),font=\upshape]
        \item $(\id_A,(\fun{refl}_{\id_A},\fun{refl}_{\id_A}))\circ(f^{-1},(\eta_f,\vartheta_f))\equiv(f^{-1}
            ,(\eta_f,\vartheta_f\ct\fun{refl}_{\id_A}))$

        \item $(f^{-1},(\eta_f,\vartheta_f))\circ(\id_A,(\fun{refl}_{\id_A},\fun{refl}_{\id_A}))\equiv(f^{-1}
            ,(\eta_f\ct\fun{refl}_{\id_B},\vartheta_f))$
    \end{enumerate}
\end{cor}
...
\begin{proof}${}$
    \begin{enumerate}[a)]
        \item By the proposition,
        \begin{align*}
            \lhs&\equiv(f^{-1},\eta,\vartheta)\\[1mm]
            \eta&\equiv(f\cdot\fun{refl}_{\id_A}\cdot f^{-1})\ct\eta_f\\
                &\equiv\fun{refl}_{\id_A}\ct\eta_f
                    &&\text{; Rem.~\ref{rem:whiskering-with-id}}\\
                &\equiv\lambda x.\,\fun{refl}_x\ct\eta_f(x)\\
                &\equiv\lambda x.\,\eta_f(x)\\
                &\equiv\eta_f.\\[1mm]
            \vartheta&\equiv(\id_A\cdot\vartheta_f\cdot\id_A)
                    \ct\fun{refl}_{\id_A}\\
                &\equiv\vartheta_f\ct\fun{refl}_{\id_A}
        ...
        \begin{align*}
            ...
            \eta&\equiv(\id_A\cdot\eta_f\cdot\id_A)
                    \cdot\fun{refl}_{\id_A}\\
                &\equiv\eta_f\ct\fun{refl}_{\id_A}.
                    &&\text{; Prop. \ref{prop:whiskering}}\\[1mm]
            \vartheta&\equiv
                (f^{-1}\cdot\fun{refl}_{\id_A}\cdot f)
                    \ct\vartheta_f\\
                &\equiv\vartheta_f
                    &&\text{; Idem part a)}
        \end{align*}
