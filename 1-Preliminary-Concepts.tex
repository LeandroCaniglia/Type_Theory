...
\begin{lpar}{The syntactic nature of Type Theory.}
    For a mathematician trained in classical structures (like groups or geometries), the definition of Homotopy Type Theory can seem unusual. Typically, mathematical structures are defined \textit{axiomatically} or \textit{descriptively}: we assume a background universe (usually ZFC Set Theory) and specify a list of properties that a collection of objects must satisfy (e.g., ``A Projective Plane is a set of points and lines such that \dots'').

    However, Type Theory is not a structure \textit{within} mathematics; it is a \textit{foundation for} mathematics. As such, it cannot rely on a pre-existing universe of sets. Instead, it must be defined \textsl{syntactically} or \textsl{generatively}, much like a programming language.

    This approach relies on three components:
    \begin{enumerate}
        \item \textbf{The Alphabet:} A finite set of primitive symbols (e.g., variables $x$, $y$), separators (e.g., `$:$' , `$($', `$)$'), and keywords like $\lambda, \Pi$).
    ...
    Therefore, when we ask \textit{What is a type in} HoTT?, the answer is not given by describing its properties in Set Theory, but by specifying the \textit{inference rules} that govern how to construct it, how to construct its elements (introduction), and how to use them (elimination).
...
\begin{lpar}{Precedence.}
    Syntactically, the precedence of `$:$' and `$\equiv$' is the lowest in an expression. The reason is that both symbols are used for judgments, which cannot be put together into more complicated statements. Thus, $A\equiv x\eq{}y$ means $A\equiv(x\eq{}y)$ since $x\eq{}y$ is a type and a judgment (like $A\equiv x$) cannot be equal to anything.\footnote{In fact, judgments cannot be terms in any propositional equality.}
\end{lpar}
...
\begin{lpar}{Curried form.}\label{lpar:currying}
    If $f\colon A\times B\to C$ is a function, its \textsl{Curried form} is the function $g\colon A\to(B\to C$) defined as $g(a)(b)\equiv f(a,b)$. The equivalence between a function and its Curried form is extended inductively to the case where the domain is $A_1\times\cdots\times A_n$.
...
\begin{lpar}{Universes.}\label{lpar:universes}
    As every term must have a type, types must have a type as well. To avoid paradoxical situations derived from the global universe, there is a family of universes $\univ_0$, $\univ_1$, $\univ_2,\dots$, such that $\univ_i\colon\univ_{i+1}$ for all $i\ge0$.\footnote{See also \S\,\ref{universes-and-type-families}.} Thus, $\univ_0$ is the universe of \textsl{small} types, such as $\N$, etc., while $\univ_{i+1}$ is the universe of types in the universe $\univ_i$.\footnote{The indexing here is distinct from the formal type of natural numbers. It is a meta-linguistic device; any finite alphabet containing symbols like `$\univ$' and a successor mark such as `$'$' would suffice, allowing us to write sequences like $\univ\colon\univ'\colon\univ''$ of arbitrary depth.}
...
\begin{lpar}{The cumulative property.}\label{lpar:cumulative-property}
    The family of universes is \textsl{cumulative}, meaning that if $A\colon\univ_i$, then $A\colon\univ_{i+1}$. In particular, if $a$ is a witness of $A$ in $\univ_{i+1}$, it is also a witness of $A$ in $\univ_i$.
    
    The following example illustrates the reason behind this rule.
    
    \begin{xmpl}
        Consider a type constructor $\fun L$ that forms the type of lists containing elements of a specific type. If we want to create a list of types, we must determine the universe in which this list lives.
        
        \begin{enumerate}[-]
            \item Let $\N\colon\univ_0$ and $\fun{Bool}\colon\univ_0$.
            
            \item We can form the list $[\N, \fun{Bool}]$. The type of this list is $\fun L_{\univ_0}$.
            
            \item However, since $\univ_0\colon\univ_1$, the type $\fun L_{\univ_0}$ itself resides in $\univ_1$.
        \end{enumerate}
        Now, suppose we have a more complex type $B$ such that $B\colon\univ_1$. If we wish to construct a list containing both $\N$ and $B$, we face a potential universe mismatch. To include $\N$ in a list of type $\fun L_{\univ_1}$, we must view $\N$ as an element of $\univ_1$.
        
        Therefore, the construction of the list $[\N,B]$ relies on the cumulative property:
        \[
            \N\colon\univ_0 \implies \N\colon\univ_1.
        \]
        This ensures that both $\N$ and $B$ are valid terms of type $\univ_1$, making the expression $[\N,B]$ well-typed as a term of $\fun L_{\univ_1}$. \qed
    \end{xmpl}
...
\begin{lpar}{Type Families.}\label{lpar:type-families}
    ...
    {\small The intuition behind a type family is that it provides a collection of types $B(x)$ indexed by the terms of a given type $A$. Defining this family as a function into $\univ$ ensures that the resulting types do not escape the range of some predefined universe. For instance, we cannot define a family such as $\lambda n.\,\univ_n$ for $n\colon\mathbb{N}$. Although this intuitively assigns a type to each natural number, the resulting types grow arbitrarily large and cannot be contained within any single, fixed universe $\univ_k$. Therefore, it cannot be typed as a function $\mathbb{N}\to\univ_k$.}
...
\begin{lpar}{Dependent functions \textrm{(a.k.a.~$\Pi$-types)}.} \label{lpar:Pi-types}
    Given a type $A\colon\univ$ and a family of types $B\colon A\to\univ$, we can construct the type of \textsl{dependent functions}
    \[
        \prod_{x\colon A}B(x)\colon\univ
    \]
    using the binder $\Pi x.\,B(x)$ (see \nref{lpar:binders}).
    
    This construction generalizes the ordinary function type $A\to B$, which is recovered by choosing the constant family $\lambda x\colon A.\,B$. This yields the definitional equality
    \begin{equation}\label{eq:function=independent-dependent-function}
        \Bigl(\prod_{x\colon A}B\Bigr)\equiv A\to B.
    \end{equation}
    As with ordinary functions, applying a dependent function $f\colon\prod_{x\colon A}B(x)$ to an argument $a\colon A$ yields an element $f(a)\colon B(a)$. This application satisfies the following computation rule: if $a\colon A$ and $f\defeq\lambda x.\,\Phi$, then both $f(a)\equiv\Phi'$ and $(\lambda x.\,\Phi)(a)\equiv\Phi'$. Here, the term $\Phi'$ is the expression obtained by replacing all free occurrences of $x$ in $\Phi$ with $a$, performing appropriate renaming to avoid variable capture (see \nref{lpar:bound-variables}).\footnote{This capture-avoiding substitution is usually denoted by $\Phi[a/x]$, meaning $f(a)\equiv\Phi[a/x]$.}
...
\begin{lpar}{Polymorphic functions.}
    ...
    \[
        \fun{swap}_{A,B,C}(f)(b,a)\equiv f(a,b).
    \]
    Note that given $f\colon\prod_{x\colon A}\prod_{y\colon B(x)}C(x,y)$, we have $f(a,b)\colon C(a,b)$.
\end{lpar}
...
\begin{lpar}{Unit type.}\label{lpar:unit-type}
    The \textsl{unit type} is the nullary product type (i.e., the cartesian product of zero types), denoted by $\fun1\colon\univ$. The \textsl{canonical} element is $\fstar\colon\fun1$.
...
\begin{lpar}{Products revisited.}\label{lpar:products-revisited}
    ...
    \begin{enumerate}[i),font=\scshape]
        ...
        \item \textit{Constructors.} Given $a\colon A$ and $b\colon B$ we have $(a,b)\colon A\times B$. In addition, $\fstar$ is the unique element of type $\fun1$.
        ...
        \[
            \fun{rec}_{\fun1}(C,c)(\fstar)\equiv c.
        \]
    \end{enumerate}
\end{lpar}
...
\begin{lpar}{Dependent functions over product.}\label{lpar:Pi-types-over-product}
    ...
    \]
    are judgmentally equal. Since $\fun{refl}_{A\times B}$ inhabits the former, it also inhabits the latter, i.e.,
    \[
    ...
    with
    \[
        \fun{uniq}_{\fun1}(\fstar)\defeq\fun{refl}_\fstar,
    \]
    which shows that $\fun\fstar$ is the unique element of $\fun1$.
\end{lpar}
...
\begin{lpar}{Induction.}\label{lpar:induction}
    ...
    \]
    \textbf{Interpretation.} The function $\fun{ind}_{A\times B}$ transforms any proof that a proposition $C$ holds for all canonical pairs $(x,y)$ into a proof of $C(z)$ for an arbitrary element $z\colon A\times B$.
    ...
    \begin{equation}\label{eq:ind1}
        \fun{ind}_{\fun1}\colon\prod_{C\colon\fun1\to\univ}
            \Bigl(C(\fstar)\to\prod_{x\colon\fun1}C(x)\Bigr)
    \end{equation}
    specified by
    \[
        \fun{ind}_{\fun1}(C,g,\fstar)\defeq g,\footnote{A more precise notation is $\fun{ind}_{\fun1}(C)(g)(\fstar)\defeq g$.}
    \]
    
    \textbf{Interpretation.} Given $C\colon\fun1\to\univ$ and a proof $g$ that $C(\fstar)$ is true, the resulting term $\fun{ind}_{\fun1}(C,g)$ ---i.e., $\fun{ind}_{\fun1}(C)(g)$--- is a dependent function $f\colon \prod_{x\colon\fun1}C(x)$ that serves as a proof for $C(x)$ for all $x\colon\fun1$.
        
    \needspace{2\baselineskip}
    In summary, we have:
    \begin{enumerate}[label=\roman*),font=\scshape]
        \item \textit{Type:} There is a type called $\fun1$.
        \item \textit{Constructor:} There is a term $\fstar\colon\fun1$.
        \item \textit{Eliminator:} There is a function $\fun{ind}_{\fun1}$ with type signature \eqref{eq:ind1}.
        \item \textit{Computation Rule:} $\fun{ind}_{\fun1}(C,g,\fstar)\defeq g$.
    \end{enumerate}
    ...
    \[
        C\defeq\lambda x.\,(x\eq{\fun1}\fstar)
    \]
    and for $g\colon C(\fstar)$ we take $\fun{refl}_\fstar$, then the result is
    \[
        \fun{ind}_{\fun1}((\lambda x.
            \,x\eq{\fun1}\fstar)
            ,\fun{refl}_\fstar
            ,\fstar)\equiv\fun{refl}_\fstar,
    \]
    which is judgmentally equal to $\fun{uniq}_{\fun1}(\fstar)$ (see \nref{lpar:Pi-types-over-product}). Thus,
    \begin{equation}\label{eq:uniq1-from-ind1}
        \fun{ind}_{\fun1}\bigl(\lambda x.\,x \eq{\fun1} \fstar,\;\fun{refl}_\fstar\bigr)
...
\begin{lpar}{Dependent pairs \textrm{(a.k.a.~$\Sigma$-types)}.}
    ...
    \[
        \Bigl(\sum_{x\colon A}B(x)\Bigr)\colon\univ.
    \]
    ...
    \[
        \dep{a,b}\colon \sum_{x\colon A}B(x).
    \]
    In this regard, $\Sigma$-types resemble the existential binder $\exists$ of Set Theory in that its elements $\dep{a,b}$ consist of a first component $a$ and a second $b$, which is a proof of $B(a)$. In particular, in the case where $B$ is constant, we have
    \[
...
\begin{lpar}{Induction in $\Sigma$-types.}\label{lpar:Sigma-induction}
    ...
    \[
        f\colon\prod_{p\colon\sum_{x\colon A}
            B(x)}\!\!\!\!\!C(p)
    \]
    we must provide a function
    \[
        g\colon\prod_{a\colon A}\;\prod_{b\colon B(a)}
            C(\dep{a,b})
    \]
    and specify
    \[
    ...
    \[
        g(a)(b) \defeq b
    \]
    for $\dep{a,b}\colon\sum_{x\colon A}B(x)$, and obtain \eqref{eq:Sigma-pi-2}. This is valid because
    \[
        C(\dep{a,b})\equiv B(\pi_1(\dep{a,b}))\equiv B(a)
    \]
    and $b\colon B(a)$.
\end{lpar}
...
\begin{lpar}{Recursion in $\Sigma$-types.}\label{lpar:Sigma-type-rec-and-ind}
    ...
    \begin{equation}\label{eq:Sigma-type-rec-evaluation}
        \fun{rec}_{\sum_{x\colon A}B(x)}(C,g,\dep{a,b})
            \defeq g(a)(b),
    \end{equation}
    ...
    \end{tikzcd}
    \]
    where $\iota_a\defeq\lambda y\colon B(a).\,\dep{a,y}$.
                ...
                \to\univ}
                    \Bigl(
                        ...
                        \prod_{b\colon B(a)}
                            \!\!\!C(\dep{a,b})
                    \Bigr)
                    \to\prod_{p\colon\sum_{x\colon A}B(x)}
                        \!\!\!\!\!C(p)
    \]
    where
    \[
        \fun{ind}_{\sum_{x\colon A}B(x)}
            (C,g,\dep{a,b})\defeq g(a)(b).
    \]
    Note that when $C$ is constant $\fun{ind}$ reduces to $\fun{rec}$.

    \textbf{Interpretation.} To prove $C(p)$ for every $p\colon\sum_{x\colon A}B(x)$ it suffices to show a proof of $C(\dep{a,b})$ for every canonical pair $\dep{a,b}$.
\end{lpar}
...
\begin{lpar}{Relations.}\label{lpar:relations}
    ...
    \]
    represents the fiber at $a$ of the relation $R$. Its elements are pairs $\dep{b,\omega}$ where $b\colon B$ and $\omega\colon R(a,b)$ is a witness that $b$ is related to $a$.
    ...
    \[
        \dep{6,\omega}\colon\sum_{n\colon\N}R(2,n).
    \]
    However, there is no term $\omega$ for which $\dep{3,\omega}\colon\sum_{n\colon\N}R(2,n)$, because the type $R(2,3)$ is uninhabited (there is no proof of $2\mid 3$).    
\end{lpar}
...
\begin{lpar}{Type-theoretic Axiom of Choice.}\label{lpar:axiom-of-choice}
    ...
    \]
    For $x\colon A$, the term $g(x)$ is a pair $\dep{y,\omega}$ where $y\colon B$ and $\omega\colon R(x,y)$. We can therefore define the \textit{choice function\/} $f\colon A\to B$ as
    \[
    ...
    \textbf{Note.} Even though both the conventional existence binder $\exists\,y\,R(x,y)$ and the dependent pair type binder $\sum_yR(x,y)$ express the existence of an element~$y$, their semantics differ. While the former predicates the mere existence of $y$, the latter identifies pairs $\dep{y,w}$, where $w$ \textit{proves\/} $R(x,y)$. Thus, in Type Theory, when we evaluate the antecedent function $g$ at $x$, the result $g(x)\equiv\dep{y,w}$ already contains the element $y$ whose existence is claimed. Consequently, the \textit{choice} is trivial in this setting. This explains why, even though the translation to Logic reads as the classical Axiom of Choice, the Type Theoretic version is a theorem whose proof is a direct consequence of the definitions.

    Incidentally, there is a backward map defined by
    \[
        \dep{f,h}\defeq\lambda x.\,\dep{f(x),h(x)},
    \]
    which is well-typed because $f(x)\colon B$ and $h(x)\colon R(x,f(x))$.
\end{lpar}

\begin{lpar}{Mathematical structures.}
    Many mathematical structures are compositions of the simplest one, called magma, that consists of a set $A$ and an operation $m\colon A\times A\to A$. In Type Theory notation the pair $\dep{A,m}$ is an element of a dependent pair type
    \[
    ...
    \]
    and, consequently, we will have expressions like $\dep{A,(m,e)}\colon PM$. By abuse of notation, however, we will sometimes simply say \textit{let $A$ be a magma/group/etc.} as it is usually expressed in the regular mathematical jargon.
\end{lpar}
...
\begin{lpar}{Coproduct recursion.}\label{lpar:coproduct-recursion}
    ...
    \textbf{Digression.} In Gentzen-style notation, the empty coproduct is denoted by\/ $\bot$ and the\/ $\fun{rec}_{\fun0}$ function is called\/ $\fun{abort}$. In that framework, the rule corresponding to \eqref{eq:rec0} is the elimination rule, which is presented as an inference rule in fractional form:
    \[
        \frac{p\colon\bot}{\fun{abort}_A\,p\colon A}
            \quad(\bot\text{\small\,Elim}).
    \]
...
\begin{lpar}{Coproduct induction.}\label{lpar:coproduct-induction}
    ...
    \begin{equation}\label{eq:coproduct-ind}
        \fun{ind}_{A+B}\colon\prod_{C\colon(A+B)\to\univ}\;
            \Bigl(\prod_{x\colon A}C(\fun{inl}(x))\Bigr)
            \to
            \Bigl(\prod_{y\colon B}C(\fun{inr}(y))\Bigr)
            \to
...
\begin{lpar}{Case Analysis on Coproducts.}
    When reasoning informally on a property $P(s)$ for an arbitrary element $s\colon A+B$ it is usual to apply \textit{case analysis\/}: one considers the scenario where $s$ is of the form $\fun{inl}(a)$ for some $a\colon A$, and separately the scenario where $s$ is of the form $\fun{inr}(b)$ for some $b\colon B$.
    
    This intuitive procedure is formally justified by the induction principle of the coproduct. Specifically, to construct a global witness:
    \[
        f\colon\prod_{s\colon A+B}P(s),
    \]
    \needspace{2\baselineskip}
    it suffices to provide two independent case branches:
    \begin{enumerate}[-]
        \item a proof for left injections: $g_0\colon\prod_{a\colon A}P(\fun{inl}(a))$, and

        \item a proof for right injections: $g_1\colon\prod_{b\colon B}P(\fun{inr}(b))$.
    \end{enumerate}
    The witness $f$ is then formally defined as
    \[
        f\defeq\lambda s.\,\fun{ind}_{A+B}(P,g_0,g_1,s).
    \]
    The computation rules:
    \begin{align*}
        f(\fun{inl}(a))&\equiv g_0(a)\\
        f(\fun{inr}(b))&\equiv g_1(b)
    \end{align*}
    guarantee that evaluating $f$ on any canonical element reduces to the proof provided for that specific case. Thus, what manifests informally as \textit{pattern matching on constructors\/} is the production of a dependent function via induction.
\end{lpar}
...
\begin{lpar}{Exactly two booleans.}\label{lpar:exactly-two-booleans}
...
\begin{lpar}{Coproducts as Indexed Disjoint Unions.}\label{lpar:coproduct-as-disjoint-union}
    ...
    \begin{equation}\label{eq:inA-inB}
        \begin{aligned}
            \fun{in}_A\colon A&\to A\oplus B, &a&\mapsto\dep{0_{\fun2},a}\\
            \fun{in}_B\colon B&\to A\oplus B, &b&\mapsto\dep{1_{\fun2},b}.
        \end{aligned}
    \end{equation}
    To see that these are well-defined specifications, note that for any $a\colon A$, the pair $\dep{0_{\fun2},a}$ has type $\sum_{x\colon\fun2}\sigma(x)$ because the second component is of the required type $\sigma(0_{\fun2})\equiv A$. A similar argument holds for $\fun{in}_B$ since $\sigma(1_{\fun2}) \equiv B$.
    ...
    \end{align*}
    To verify the first equation, by the definition of $A\oplus B$, it suffices to consider the cases $p\defeq\dep{0_{\fun2},a}$ and $p\defeq\dep{1_{\fun2},b}$, with $a\colon A$ and $b\colon B$, since these exhaust all the canonical pairs of $A\oplus B$.
    \begin{itemize}
        \item For $p\defeq\dep{0_{\fun2},a}$, we have $\psi(\dep{0_{\fun2},a}) \equiv h(0_{\fun2})(a) \equiv \fun{inl}(a)$. Thus,
        \[
            \phi(\psi(p))
                \equiv\phi(\fun{inl}(a))
                \equiv\fun{in}_A(a)
                ...
                \equiv p.
        \]
        \item For $p\defeq\dep{1_{\fun2},b}$, we have $\psi(\dep{1_{\fun2},b})\equiv h(1_{\fun2})(b)\equiv\fun{inr}(b)$. Thus,
        \[
            \phi(\psi(p))
                \equiv\phi(\fun{inr}(b))
                \equiv\fun{in}_B(b)
    ...
    \begin{itemize}
        \item For $s\defeq\fun{inl}(a)$, $\psi(\phi(s)) \equiv\psi(\fun{in}_A(a))\equiv\psi(\dep{0_{\fun2},a})\equiv\fun{inl}(a)\equiv s$.
        \item For $s\defeq\fun{inr}(b)$, $\psi(\phi(s)) \equiv\psi(\fun{in}_B(b))\equiv\psi(\dep{1_{\fun2},b})\equiv\fun{inr}(b)\equiv s$.
    \end{itemize}
...
\begin{lpar}{Pattern matching.}
    ...
    \needspace{2\baselineskip}
    \begin{xmpl}
        Let us define the factorial function $\fun{fact} : \N \to \N$.
        ...
        \begin{enumerate}[a),font=\upshape]
            \item The target type is $C \equiv\N$.
...
\begin{lpar}{Propositional Logic.}\label{lpar:propositional-logic}
    ...
    \begin{enumerate}[$1.$]
        \item Since $\fun\fstar\colon\fun1$, we see that $\fun1$ has a proof (it is always inhabited).
...
\begin{rem}\label{rem:paths-as-2-cells}
    A point\/ $x\colon A$ can be interpreted as a function from the unit type, \/ $x\colon\fun1\to A$. Under this convention, a path\/ $p\colon x\eq Ay$ can be seen as a commutative diagram: a homotopy between the maps\/ $x$ and\/ $y$.
    \[
        \begin{tikzcd}
            \fun1
...
\begin{lpar}{General path induction.}\label{lpar:path-induction}
    ...
    \]
    This distinction explains why we write $C(x,y,p)$ rather than $C(\dep{(x,y),p})$.
    ...
    \]
    for every $a\colon A$.\footnote{This mirrors the categorical Yoneda's lemma~\ref{thm:yoneda-general}, which states that natural transformations from a representable functor $\Hom(x,-)$ are completely determined by evaluating at the identity morphism on $x$.}
\end{lpar}
...
\begin{lpar}{Path concatenation.}\label{lpar:path-concatenation}
    ...
    \begin{lem}\label{lem:path-concatenation}
        ...
        written
        \[
            \qquad\qquad p\mapsto q\mapsto p\ct q
        \qquad
            \begin{tikzcd}[column sep=2cm]
                \fun1
                        \arrow[r,"x",bend left=50]
                        \arrow[r,"y" description]
                        \arrow[r,"z"',bend right=50]
                        \arrow[r,phantom,"\scriptstyle p",description,bend left=25]
                        \arrow[r,
                            phantom,
                            "\scriptstyle q",
                            description,
...
\begin{lpar}{Right unit law.}\label{lpar:right-unit-law}
    ...
    \begin{lem}\label{lem:right-unit-law}
        ...
        \[
            \fun{ru}(p)\colon p\ct\fun{refl}_y\eq{x\eq Ay}p.
        \]
    \end{lem}
    ...
    \begin{proof}
        ...
        \[
            \fun{ru}(p) \defeq \fun{ind}_{\eq A}(C,c,x,y,p).
        \]
    \end{proof}
\end{lpar}
...
\begin{thm}\label{thm:transportP}
...
\begin{lpar}{Based path induction.}\label{lpar:based-path-induction}
    A different approach to path induction, which we will show is equivalent to the general one, consists of fixing an element\/ $a\colon A$ before considering a type family\/ $D$ specific to\/ $a$,
    \[
        D\colon\prod_{x\colon A}(a\eq{A}x)\to\univ.
    \]
    Given an element\/ $d\colon D(a,\fun{refl}_a)$, we then obtain a dependent function
    \[
        f\colon\prod_{x\colon A}\;\prod_{p\colon a\eq{A}x}D(x,p).
    \]
...
\begin{lpar}{Path induction equivalence.}
    ...
    \]
    By \nref{lpar:right-unit-law}, there is a path  $\fun{ru}(s)\colon s\ct\fun{refl}_u\eq{a\eq Au}s$. Then, its inverse path satisfies  $\fun{ru}(s)^{-1}\colon s\eq{a\eq Au}s\ct\fun{refl}_u$.
    ...
    \begin{equation}\label{eq:ind-base-case-subst}
        c(u)\defeq\lambda s.\,\fun{transport}^{P_u}(\fun{ru}(s)^{-1})
    ...
    \begin{align*}
        c(a)(\fun{refl}_a)
            &\equiv\fun{transport}^{P_a}(\fun{ru}(\fun{refl}_a)^{-1})
                &&;\ \eqref{eq:ind-base-case-subst}\\
            &\equiv\fun{transport}^{P_a}(\fun{refl}_a^{-1})
                &&;\ \fun{ru}(\fun{refl}_a)\equiv\fun{refl}_a\\
...
\begin{lpar}{Successor injectivity.}
    ...
    \begin{proof}
        ...
        \end{align*}
        Take $n\colon\N$ and suppose we have a path $p\colon\fun{succ}(n)\eq\N0$. By transport along the inverse path $p^{-1}\colon0\eq\N\fun{succ}(n)$, for $\fstar\colon\fun1$, we obtain:
        \[
            \fun{transport}^P(p^{-1})\colon P(0)\to P(\fun{succ}(n)).
        \]
        The definition of $P$ implies that the desired map is $(n,p)\mapsto\fun{subst_P}(p^{-1})(\fstar)$.
    \end{proof}
...
\begin{lpar}{The order relation on $\N$.}\label{lpar:order-in-N}
    The translation table (see \nref{lpar:predicate-logic}) implies that the order relations can be defined as
    \begin{align*}
    ...
    \begin{lem}\label{lem:order-in-N}
        The following properties hold
        \begin{enumerate}[a),font=\upshape]
            \item $0\le n$
            \item $n\le\fun{succ}(n)$
            \item $n\le n$
            \item $n\le m$ and\/ $m\le n$ imply\/ $n\eq\N m$
            \item $n\le m$ and\/ $m\le r$ imply\/ $n\le r$
            \item $n\le m$ or\/ $m\le n$
            \item $n<0$ does not hold
            \item $\neg(n\eq\N0)$ implies\/ $n\eq\N\fun{succ}(\fun{pred}(n))$
        \end{enumerate}        
    \end{lem}
    \begin{proof} (Sketch)
        \begin{enumerate}[a)]
            \item Take $k \defeq n$. By the definition of addition (see \nref{lpar:natural-addition}), we have the definitional equality $0+n \equiv n$. Thus, $\fun{refl}_n$ has type $0+n \eq\N n$. The proof term is $\dep{n,\fun{refl}_n}$.
            ...
            The base case $n \eq{\N} 0$ is covered by part a).
            
            For the inductive step, assume that for a fixed $n\colon\N$ and all $m\colon\N$, we have $(n \le m) + (m \le n)$. We must show that for all $m\colon\N$, we have
            \[
                (\fun{succ}(n) \le m) + (m \le \fun{succ}(n)).
            \]
            We proceed by induction on $m$.
            
            For $m \eq{\N} 0$, it suffices to note that $0 \le \fun{succ}(n)$.
            
            For the inductive step, assume the result holds for $m$, so we want to show
            \[
                (\fun{succ}(n) \le \fun{succ}(m)) + (\fun{succ}(m) \le \fun{succ}(n)). \tag{1}
            \]
            By the outer inductive hypothesis applied to $m$, we have a witness of
            \[
                (n \le m) + (m \le n). \tag{2}
            \]
            To construct a map from the type defined in (2) to the type defined in (1), it suffices to define two maps, one from $n \le m$ and another from $m \le n$, both of which are easily deduced by applying $\fun{succ}$.
            
            This completes the induction on $m$, and therefore the induction on $n$.

            \item By definition $(n<0)\equiv(n\le0)\times\neg(n\eq\N0)$. Thus, if $u\colon n<0$ then $\pi_1(u)\colon n\le0$ and, by part~d), there exists $p\colon n\eq\N0$. Thus, $\pi_2(u)(p)\colon\fun0$, proving $\neg(n<0)$.

            \item Let $u\colon(n\eq0)\to\fun0$. We proceed by induction on $n\colon\N$.
    
            In the case $n\equiv0$, the goal is obtained as
            \[
                \fun{rec}_{\fun0}
                    \bigl(
                        0\eq\N\fun{succ}(\fun{pred}(0)), u(\fun{refl}_0)
                    \bigr).
            \]
            
            For the inductive step, assume the property holds for $n\colon\N$. We must construct a function for $\fun{succ}(n)$ with signature
            \[
                \neg(\fun{succ}(n)\eq\N0)
                    \to
                    \bigl(
                    \fun{succ}(n)\eq\N\fun{succ}(\fun{pred}(\fun{succ}(n))
                    \bigr).
            \]
            Assume $u\colon\neg(\fun{succ}(n)\eq\N0)$. By definition, $\fun{pred}(\fun{succ}(n))\equiv n$. Therefore, our goal reduces definitionally to:
            \[
                \fun{succ}(n)\eq\N\fun{succ}(n),
            \]
            which is trivially inhabited by $\fun{refl}_{\fun{succ}(n)}$, regardless of~$u$ and the inductive hypothesis.
            
        \end{enumerate}
    \end{proof}
\end{lpar}

\section{Summary of Constructors and Eliminators}

\begin{enumerate}[-]
    \item $A\times B$
    
    \textsc{Constructors:} $(a,b)$
    
    \textsc{Eliminators:} $\pi_1, \pi_2$
    \begin{align*}
        \beta & \colon \begin{cases}
            \pi_1(a,b) \equiv a \\
            \pi_2(a,b) \equiv b
        \end{cases} \\
        \eta & \colon z \equiv (\pi_1(z), \pi_2(z))
    \end{align*}

    \item $\prod_{x:A} B(x)$
    
    \textsc{Constructors:} $\lambda x.\,\Phi$
    
    \textsc{Eliminators:} $\fun{ev}(f,a)\equiv f(a)$
    \begin{align*}
        \beta&\colon(\lambda x.\,\Phi)(a)\equiv\Phi[x\leftarrow a]\\
        \eta & \colon f \equiv \lambda x.\, f(x)
    \end{align*}

    \item $\sum_{x:A}B(x)$
    
    \textsc{Constructors:} $\dep{a,b}$
    
    \textsc{Eliminators:} $\pi_1, \pi_2$
    \begin{align*}
        \beta&\colon\begin{cases}
            \pi_1(\dep{a,b})\equiv a\\
            \pi_2(\dep{a,b})\equiv b
        \end{cases}\\
        \eta&\colon z\equiv\dep{\pi_1(z),\pi_2(z)}
    \end{align*}

    \item $A+B$
    
    \textsc{Constructors:} $\fun{inl},\fun{inr}$
    
    \textsc{Eliminators:} $\fun{ind}_{A+B}$ (case analysis)
    \begin{align*}
        \beta & \colon \begin{cases}
            \fun{ind}_{A+B}(\fun{inl}(a), l, r) \equiv l(a) \\
            \fun{ind}_{A+B}(\fun{inr}(b), l, r) \equiv r(b)
        \end{cases} \\
        \eta & \colon z \equiv \fun{ind}_{A+B}(z, \fun{inl}, \fun{inr})
    \end{align*}

    \item $\fun{2}$
    
    \textsc{Constructors:} $0_{\fun2},1_{\fun2}$
    
    \textsc{Eliminators:} $\fun{ind}_{\fun2}$ (if-else)
    \begin{align*}
        \beta&\colon\begin{cases}
            \fun{ind}_{\fun2}(0_{\fun2},c_0,c_1)\equiv c_0\\
            \fun{ind}_{\fun2}(1_{\fun2},c_0,c_1)\equiv c_1
        \end{cases} \\
        \eta&\colon x\equiv\fun{ind}_{\fun2}(x,0_{\fun2},1_{\fun2})
    \end{align*}

    \item $\N$
    
    \textsc{Constructors:} $0,\fun{succ}$
    
    \textsc{Eliminators:} $\fun{rec}_\N$ (recursion)
    \begin{align*}
        \beta & \colon \begin{cases}
            \fun{rec}_\N(0, c_0,c_s)\equiv c_0\\
            \fun{rec}_\N(\fun{succ}(n),c_0,c_s)
                \equiv c_s(n,\fun{rec}_\N(n,c_0,c_s))
        \end{cases}\\
        \eta&\colon n\equiv\fun{rec}_\N(n,0,\fun{succ})
    \end{align*}
\end{enumerate}
...
\begin{solution}
    ...
    \[
        \fun{rec}_{\sum_{x\colon A} B(x)}\colon
            \prod_{C\colon\univ}\Bigl(\prod_{x\colon A}B(x)
                \to C\Bigr)
                \to \Bigl(\sum_{x\colon A}B(x)\Bigr)
                \to C
    \]
    ...
    It remains to be shown that the recursion works as expected. To verify this we have to evaluate the function at canonical elements. In this case it suffices to compute the $\lambda$-expression when $p\equiv(a,b)$ with $b\colon B(a)$. Here we have
    \begin{align*}
        \fun{rec}_{\sum_{x\colon A}B(x)}(C,g)(\dep{a,b})
            &\equiv
                \bigl(
                    \lambda p.\,g(\pi_1(p))(\pi_2(p))
                \bigr)(\dep{a,b})\\
            &\equiv g(\pi_1(\dep{a,b}))(\pi_2(\dep{a,b}))\\
            &\equiv g(a)(b),
    \end{align*}
    because $\pi_1(\dep{a,b})\equiv a$ and $\pi_2(\dep{a,b})\equiv b$. Since this coincides with \eqref{eq:Sigma-type-rec-evaluation}, our definition of $\fun{rec}$ recovers the definition given in \nref{lpar:Sigma-type-rec-and-ind}.
...
\begin{solution}
    ...
    However, strictly speaking, that would be too much for our needs, since the equality $q_1(n)\eq\N n$ for $n\colon\N$ would imply by congruence\footnote{See Exercise~\ref{exr:ap-application}.} that $\fun{rec}$ satisfies the recursion principle equations propositionally, which is what we want.
...
\begin{exr}\label{exr:A+B-from-rec2}
...
\begin{solution}
    This is a complement to \nref{lpar:coproduct-as-disjoint-union}, where the notation for the coproduct is $A\oplus B$. To formulate the induction principle, we must first define the injection functions for this specific $\Sigma$-type construction. The definitional equalities
    \[
        \fun{rec}_{\fun2}(\univ,A,B,0_{\fun2})\equiv A
        \quad\text{and}\quad
        \fun{rec}_{\fun2}(\univ,A,B,1_{\fun2})\equiv B,
    \]
    imply that we can define the injections into $A\oplus B$ as
    \[
        \fun{in}_A(x)\defeq\dep{0_{\fun2},x}
        \quad\text{and}\quad
        \fun{in}_B(y)\defeq\dep{1_{\fun2},y}.
    \]
    ...
    \]
    where the motive $D\colon\fun2\to\univ$ is given by
    \[
        D(t)\defeq\prod_{z\colon\fun{rec}_{\fun2}(\univ,A,B,t)}C(\dep{t,z}).
    \]
    Applying the definitional equalities of $\fun{rec}_{\fun2}$, we obtain $D(0_{\fun2})\equiv \prod_{x\colon A}C(\fun{in}_A(x))$ and $D(1_{\fun2})\equiv \prod_{y\colon B}C(\fun{in}_B(y))$.

    Finally, we must verify that the definitional equalities hold. For any $x\colon A$, the evaluation rules for the $\Sigma$-type and the boolean type yield
    \begin{align*}
        f(\fun{in}_A(x)) 
        &\equiv f(\dep{0_{\fun2}, x}) \\
        &\equiv g(0_{\fun2}, x) \\
        &\equiv \fun{ind}_{\fun2}(D,g_0,g_1,0_{\fun2})(x) \\
        &\equiv g_0(x).
    \end{align*}
    By a symmetric argument, for any $y\colon B$, we have $f(\fun{in}_B(y))\equiv g_1(y)$. This completes the construction.
\end{solution}
...
\begin{exr}${}$
    \begin{enumerate}[a),font=\upshape]
        ...
        \item Verify that\/ $(\N,+,0,\dummy,1)$ is a semiring using only\/ $\fun{ind}_\N$.
...
\begin{solution}
    ...
    \[
        \fun{ack}(m,\dummy)\defeq\fun{rec}_\N(C,c_0,c_s,m)
    \]
    we obtain
    \begin{align*}
        \fun{ack}(0,n)&\equiv c_0(n)\\
        \fun{ack}(\fun{succ}(m),\dummy)
            &\equiv c_s(m,\fun{ack}(m,\dummy))
    \end{align*}
    ...
    To define $c_s(m,g)\colon\N\to\N$, we fix $m\colon\N$ and $g\colon\N\to\N$, with $g$ playing the role of $\fun{ack}(m,\dummy)$, and proceed by recursion on $n$. We need to provide two pieces of information
    \begin{align*}
        d_0&\colon\N\\
        d_s&\colon\N\to\N\to\N,
    \end{align*}
    where $d_0\equiv c_s(m,g)(0)$. Since $c_s$ is the step function on $m$ for $\fun{ack}(m,\dummy)$, we deduce that
    \[
        d_0 \equiv \fun{ack}(\fun{succ}(m),0)\equiv\fun{ack}(m,1).
    \]
    Therefore, we must specify $d_0\defeq g(1)$.

    Since $c_s(m,g)$ represents $\fun{ack}(\fun{succ}(m),\dummy)$, and
    \[
...
\begin{exr}\label{exr:neg-neng-neg}
...
\begin{solution}
    ...
    {\small
    \textit{Second solution:} Consider the evaluation $\ev{}\colon A\to\neg\neg A$ defined by $a\mapsto\lambda\zeta\colon\neg A.\,\zeta(a)$. Now observe the functoriality of $\neg$, i.e., every map $X\to Y$ produces by composition another map $\neg Y\to\neg X$.
    }
\end{solution}
...
\begin{exr}
    ...
    \[
        f\colon\prod_{x\colon A}\;\prod_{p\colon x\eq Ax} (p\eq{x\eq Ax}\fun{refl}_x)
    \]
...
\begin{solution}
    ...
    The given expression only considers paths of the form $p\colon x\eq Ax$, between $x$ and itself, while the induction principle requires paths between two variables.
\end{solution}
