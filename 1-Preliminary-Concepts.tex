...
\begin{lpar}{Generalized Currying.}\label{lpar:generalized-currying}
    ...
    \[
        f\colon\Big(\sum_{x\colon A}B(x)\Big)\to C
    \]
...
\begin{lpar}{Induction in $\Sigma$-types.}\label{lpar:Sigma-induction}
    ...
    \[
        \pi_1\colon\Big(\sum_{x\colon A}B(x)\Big)
            \to A
    \]
    with specification
    \[
...
\begin{lpar}{Recursion in $\Sigma$-types.}\label{lpar:Sigma-type-rec-and-ind}
    ...
    \[
        \fun{rec}_{\sum_{x\colon A} B(x)}\colon
            \prod_{C\colon\univ}\Big(\prod_{x\colon A}
                B(x)\to C\Big)
                \to\Big(\sum_{x\colon A}B(x)\Big)
                \to C
    \]
    specified as
    ...
    \[
        \fun{ind}_{\sum_{x\colon A}B(x)}\colon
            \prod_{C\colon\big(\sum_{x\colon A}B(x)\big)
                \to\univ}
                    \Big(
...
\begin{lpar}{Type-theoretic Axiom of Choice.}\label{lpar:axiom-of-choice}
    ...
    \[
        \fun{ac}\colon\Big(\prod_{x\colon A}\;\sum_{y\colon B}
            R(x,y)\Big)\to\Big(\sum_{f\colon A\to B}\;\prod_{x\colon A}R(x,f(x))\Big).
    \]
...
\begin{lpar}{Coproducts as Indexed Disjoint Unions.}\label{lpar:coproduct-as-disjoint-union}
    ...
    \[
        \fun{rec}_{\sum_{x\colon\fun2} \sigma(x)}(A+B)\colon
            \Big(\prod_{x\colon\fun2}\sigma(x)
                \to A+B\Big)
                \to \Big(\sum_{x\colon\fun2}\sigma(x)\Big)
                \to A+B
    \]
...
\begin{lpar}{Induction principle.}\label{lpar:induction-principle}
    ...
    \[
        \fun{ind}_\N\colon\prod_{C\colon\N\to\univ}
            C(0) \to \Big(\prod_{n\colon\N}C(n) \to C(\fun{succ}(n))\Big)
...
\begin{lpar}{Predicate logic.}\label{lpar:predicate-logic}
    ...
    \[
        \Big(\prod_{x\colon A}(P(x)\times Q(x))\Big)
            \to\Big(\prod_{x\colon A}P(x)\Big)\times\Big(\prod_{x\colon A}Q(x)\Big).
    \]
...
\begin{lpar}{General path induction.}
    ...
    \[
        \Big(\sum_{x,y\colon A}x\eq A y\Big)\to\univ.
    \]
    ...
    \begin{equation}\label{eq:ind=A}
        \fun{ind}_{\eq A}\colon\prod_{C\colon\prod_{x,y\colon A}(x\eq A y)\to\univ}
            \;\bigg(\prod_{x\colon A}C(x,x,\fun{refl}_x)\bigg)
...
\begin{lpar}{Based path induction.}
    ...
    \begin{equation}\label{eq:ind'=A}
        \fun{ind}'_{\eq A}\colon\prod_{a\colon A}\;\;
            \Bigg(
            \prod_{D\colon\prod_{x\colon A} (a\eq A x)\to\univ}D(a,\fun{refl}_a)
            \Bigg)
        \to\prod_{x\colon A}\;\prod_{p\colon a\eq Ax}D(x,p)
    \end{equation}
    with the equality
    \[
...
\begin{solution}
    ...
    \[
        \fun{rec}_{\sum_{x\colon A} B(x)}\colon
            \prod_{C\colon\univ}\Big(\prod_{x\colon A}\big(B(x)
                \to C\big)\Big)
                \to \Big(\sum_{x\colon A}B(x)\Big)
                \to C
    \]
    ...
    \[
        g\colon\prod_{x\colon A}\big(B(x)\to C\big).
    \]
