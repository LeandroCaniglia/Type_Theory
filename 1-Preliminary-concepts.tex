...
\begin{lpar}{Multiplication of natural numbers.}
    To define
    \[
        \fun{mult}\colon\N\to\N\to\N
    \]
    we apply primitive recursion with $C\defeq\N\to\N$, and specify:
    \begin{align*}
        c_0 &\defeq \lambda n.\, 0,\\
        c_s &\defeq \lambda m.\, \lambda g.\, \lambda n.\, \fun{add}(g(n), n).
    \end{align*}
    Defining $\fun{mult}\defeq\fun{rec}_\N(C,c_0,c_s)$, which verifies the following equations:
    \begin{align*}
...
\begin{lpar}{Blocked computation.}\label{lpar:blocked-computation}
    ...
    \end{itemize}
    To complete the proof, we need to apply the function $\fun{succ}$ to both sides of the hypothesis $h$. This relies on the \textit{principle of congruence} that states that functions respect equality, a lemma that we will formally introduce in the next chapter.
\end{lpar}
...
\begin{lpar}{Substitution.}\label{lpar:substitution}
    The principle of \textsl{indiscernibility of identicals} establishes that the equality relation is compatible with type substitution. 
...
\begin{solution}
    ...
    \]
    which, according with \eqref{eq:inA-inB}, satisfies $g_0\colon D(0_{\fun2})$ and $g_1\colon D(1_{\fun2})$.
    %
