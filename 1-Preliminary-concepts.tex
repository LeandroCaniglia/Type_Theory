...
\begin{lpar}{The syntactic nature of Type Theory.}
    For a mathematician trained in classical theories (like Group Theory or Geometry), the definition of Homotopy Type Theory can seem unusual. Typically, we define a mathematical structure \emph{axiomatically} or \emph{descriptively}: we assume a background universe (usually ZFC Set Theory) and specify a list of properties that a collection of objects must satisfy (e.g., ``A Projective Plane is a set $P$ of points and lines such that...'').
    ...
    \begin{enumerate}
        \item \textbf{The Alphabet:} A finite set of primitive symbols (e.g., variables $x, y$, separators `$:\,,\,(\,)$', and keywords like $\lambda, \Pi$).
    ...
    To set this generative process in motion, the theory requires a set of \textsl{primitive objects} (or base types) to serve as the \textit{seeds} of the universe. Just as a grammar needs terminal symbols to form sentences, Type Theory postulates specific starting points—such as the Empty type ($\fun{0}$), the Unit type ($\fun{1}$), and the Natural Numbers ($\N$).
...
\begin{lpar}{Definitional substitution.}\label{lpar:definitional-substitution}
    Given that $x\equiv a$ is not a type, we cannot have it as an assumption. What we can do instead is to \textsl{substitute $a\colon A$ for $x$} and obtain a more specific type or element. Even though not technically an assumption, the language ``assume $x\equiv a$'' will be used to refer to this process of substitution.
\end{lpar}

\begin{lpar}{Principles.}
    Some statements (whether a judgment, axiom, or theorem) that possess high generality and acts as fundamental building blocks for reasoning within the theory are called \textsl{principles}. This is not a formal notion, but a pedagogical or communicational one aimed at underlying its importance in the deductive system.
...
\begin{lpar}{Binders.}\label{lpar:binders}
    A \textsl{binder} (a.k.a.~\textsl{variable binder}) is an \textit{operator}~$\Omega$ that can be applied to a variable $x$ of type $A$, and an expression $E$ using the syntax
    \[
        \nabla x\colon A.\,E.
    \]
\end{lpar}
...
\begin{lpar}{Function abstraction.}
    ...
    \textbf{Note.} The mathematical notation $f(x)=\Phi$, where $\Phi$ is an expression possibly involving $x$, binds the variable $x$ to the scope of $\Phi$. However, unlike other binders such as $\forall$, $\exists$, $\Pi$, $\Sigma$, etc., this one is named after the function. The $\lambda$-notation, \textit{removes} this dependency, emphasizing the binding character of the entire expression. 
\end{lpar}
...
\begin{lpar}{Implicit notation.}
    ...
    In particular, if $f\colon A\to B\to C$ and $x\colon A$, then $f(x)$ is a function of type $B\to C$.
\end{lpar}
...
\begin{lpar}{Type Families.}\label{lpar:type-families}
    A \textsl{type family} is a function $B\colon A\to\univ$ whose codomain is a universe. There are two important examples. The first one is the \textsl{constant} type family defined by
    \[
...
\begin{lpar}{Dependent functions}\hspace{-0.5em} (a.k.a.~$\Pi$-types). \label{lpar:Pi-types}
    ...
    This type is a generalization of the function type $A\to B$, which can be recovered by taking the constant family $\lambda x\colon A.\,B$, to produce
    \[
        \prod_{x:A}B\equiv A\to B.
    \]
    As before, we can apply a dependent function $f\colon\prod_{x:A}B(x)$ to an argument $a\colon A$ and obtain an element $f(a)\colon B(a)$. In particular, we have the following computation rule: if $a\colon A$ and $f(x)\defeq\Phi$, then $f(a)\equiv\Phi'$ and $(\lambda x.\,\Phi)(a)\equiv\Phi'$, where $\Phi'$ is obtained by replacing all occurrences of $x$ in $\Phi$ by $a$, i.e., avoiding dummy variable captures (see \nref{lpar:bound-variables}).

    An example that will become useful later is the dependent function
    \[
        \fun{fmax}\colon\prod_{n:\N}\fun{Fin}(\fun{succ}(n))
    \]
...
\begin{lpar}{Polymorphic functions.}
    ...
    \[
        \fun{swap}\colon
            \prod_{A,B,C:U}(A\to B\to C)\to(B\to A\to C)
    \]
    defined as
    \[
    ...
    \]
    Note that given $f\colon\prod_{(x:A)}\prod_{(y:B(x))}C(x,y)$, we have $f(a,b)\colon C(a,b)$, assuming $a\colon A$ and $b\colon B(a)$.
\end{lpar}
...
\begin{lpar}{Products revisited.}\label{lpar:products-revisited}
    ...
    \begin{enumerate}[i),font=\scshape]
        ...
        In a diagram\footnote{Strictly speaking, the domain of the Curried function\/ $g\colon A\to B\to C$ is not a single object in the diagram, so we represent the input context explicitly.}
        \[
        ...
        specified as
        \[
            \fun{rec}_{\fun1}(C,g)\defeq\lambda\fun s.\,g(\fun\star)
        \]
...
\begin{lpar}{Recursion significance.}
    ...
    Note also that $\fun{rec}$ implements \textsl{Uncurrying}, namely $\Hom(A,B\to C)\to\\Hom(A\times B,C)$. In Logic, this corresponds to the transformation from $A\Rightarrow (B\Rightarrow C)$ to $A \wedge B\Rightarrow C$ (cf.~\nref{lpar:currying}).
...
\begin{lpar}{Dependent functions over product.}\label{lpar:Pi-types-over-product}
    According to the specification of $\Pi$-types (see \nref{lpar:Pi-types} and \nref{lpar:products-revisited}), given $A,B\colon\univ$ and a type family $C\colon A\times B\to\univ$, a dependent function
    \[
        f\colon\prod_{z:A\times B}C(z)
    \]
    ...
    \[
        ...
        \big(\pi_1((a,b)),\pi_2((a,b))\big)\eq{A\times B}(a,b)
    \]
...
\begin{lpar}{Induction.}\label{lpar:induction}
    ...
    \[
        \prod_{(x:A)}\prod_{(y:B)}C((x,y))
            \equiv A\to B\to C
        \quad\text{and}\quad
        \prod_{z:A\times B}C(z)\equiv A\times B\to C,
    \]
    ...
    \textbf{Interpretation.} Given $C\colon\fun1\to\univ$ and a proof $g$ that $C(\fun\star)$ is true, the resulting term $\fun{ind}_{\fun1}(C,g)$ ---i.e., $\fun{ind}_{\fun1}(C)(g)$--- is a dependent function $f\colon \prod_{x:\fun1}C(x)$ that serves as a proof for $C(x)$ for all $x\colon\fun1$.
    ...
    \begin{equation}\label{eq:uniq1-from-ind1}
        \fun{ind}_{\fun1}\big(\lambda x.\,x \eq{\fun1} \fun\star,\;\fun{refl}_{\fun\star}\big)
...
\begin{lpar}{Induction in $\Sigma$-types.}\label{lpar:Sigma-induction}
    ...
    \begin{equation}\label{eq:Sigma-pi-2}
        \pi_2\colon\prod_{p:\sum_{(x:A)}B(x)}B(\pi_1(p))
    \end{equation}
    specified as
    \[
        \pi_2((a,b)) \defeq b.
    ...
    \[
        f\colon\prod_{p:\sum_{x:A}B(x)}\!\!\!\!\!C(p)
    \]
...
\begin{lpar}{Recursion in $\Sigma$-types.}\label{lpar:Sigma-type-rec-and-ind}
    ...
    \textbf{Interpretation.} To prove that for every $p\colon\sum_{x:A}B(x)$ there is a proof of $C(p)$ it suffices to show a proof of $C((a,b))$ for every canonical pair $(a,b)$, where $a\colon A$ and $b\colon B(a)$.
\end{lpar}
...
\begin{lpar}{Relations.}\label{lpar:relations}
    ...
    For instance, let $R(2,n)$ stand for the type representing the relation ``$2\mid n$'', where $n\colon\N$. The elements of the type $\sum_{n:\N}R(2,n)$ represent the even numbers paired with the evidence of their evenness. Thus, if $\omega$ is a witness that $2\cdot3=_\N6$, we have
    \[
...
\section{Coproduct Types}
    \begin{lpar}{Coproducts.} Given types $A,B\colon\univ$ their \textsl{coproduct} is the type $A+B\colon\univ$, a type that corresponds to the \textit{disjoint union} in Set Theory. Its constructors consist of two maps
    \begin{align*}
    ...
    \[
        ...
        \end{tikzcd}
    \]
    This means that to specify $f$ we have to provide two maps $g_0$ and $g_1$ and define
    \[
        f(\fun{inl}(a))\defeq g_0(a)\quad\text{and}\quad
        f(\fun{inr}(b))\defeq g_1(b),
    \]
...
\begin{lpar}{Coproduct recursion.}\label{lpar:coproduct-recursion}
    ...
    with specification
    \begin{align*}
        \fun{rec}_{A+B}(C, g_0, g_1, \fun{inl}(a)) &\defeq g_0(a),\\
        \fun{rec}_{A+B}(C, g_0, g_1, \fun{inr}(b)) &\defeq g_1(b).
    \end{align*}
    Additionally,
    \[
        \fun{rec}_{\fun0}\colon\prod_{C:\univ}\fun0\to C
    \]
...
\begin{lpar}{Coproduct induction.}\label{lpar:coproduct-induction}
    ...
    \[
        f\colon\prod_{s:A+B}C(s)
    \]
    we have to provide two dependent functions
    \begin{align*}
        g_0 &\colon \prod_{x:A}C(\fun{inl}(x)),\\
        g_1 &\colon \prod_{y:B}C(\fun{inr}(y))
    \end{align*}
    ...
    \[
        \fun{ind}_{A+B}\colon\prod_{C:(A+B)\to\univ}\;
            \prod_{x:A}C(\fun{inl}(x))
            \to
            \prod_{y:B}C(\fun{inr}(y))
            \to
            \prod_{s:A+B}C(s).
    \]
    ...
    \[
        \fun{ind}_{\fun0}\colon\prod_{C:\fun0\to\univ}\;
            \prod_{s:\fun0}C(s).
    \]
...
\begin{lpar}{The type of booleans.}\label{lpar:boolean-type}
    ...
    \[
        ...
        f(1_{\fun2}) \defeq c_1.
    \]
    Therefore, the recursor is given by
    \begin{equation}\label{eq:rec2}
        \fun{rec}_{\fun2}\colon\prod_{C:\univ}C\to C\to\fun2\to C,
    \end{equation}
    specified by
    \begin{align*}
        \fun{rec}_{\fun2}(C,c_0,c_1,0_{\fun2})&\defeq c_0,\\
        \fun{rec}_{\fun2}(C,c_0,c_1,1_{\fun2})&\defeq c_1,
    \end{align*}
    which express the fact that $f\colon\fun2\to C$ is given by a pair of elements of type $C$. We can capture this in a definitionally commutative diagram
    \[
    ...
    \[
        \fun{ind}_{\fun2}\colon\prod_{C:\univ}\;
...
\begin{lpar}{Exactly two booleans.}
    ...
    \[
        \fun{ind}_{\fun2}\big(C,
            \fun{inl}(\fun{refl}_{0_{\fun2}}),
            \fun{inr}(\fun{refl}_{1_{\fun2}})\big)
            \colon\prod_{x:\fun2}
...
\begin{lpar}{Coproducts as Indexed Disjoint Unions.}\label{lpar:coproduct-as-disjoint-union}
    ...
    \[
        A \oplus B \defeq \sum_{x:\fun2} \sigma(x)
    \]
    along with
    \begin{equation}\label{eq:inA-inB}
        ...
        \end{aligned}
    \end{equation}
    To see that these are well-defined specifications, note that for any $a\colon A$, the pair $(0_{\fun2},a)$ has type $\sum_{x:\fun2}\sigma(x)$ because the second component must have type $\sigma(0_{\fun2})$. Since $\sigma(0_{\fun2})\equiv A$, $a$ is indeed a valid second component. A similar argument holds for $\fun{in}_B$ since $\sigma(1_{\fun2}) \equiv B$.
    ...
    \[
        \fun{rec}_{\sum_{x:\fun2} \sigma(x)}(A+B)\colon
            \prod_{x:\fun2}\big(\sigma(x)
                \to A+B\big)
                \to \Big(\sum_{x:\fun2}\sigma(x)\Big)
                \to A+B
    \]
    and apply the result to a well-chosen dependent map
    \[
        h\colon\prod_{x:\fun2}(\sigma(x)\to A+B).
    \]
    ...
    \begin{equation}\label{eq:psi-sigma-to-coproduct}
        \psi\defeq\fun{rec}_{\sum_{x:\fun2}\sigma(x)}(A+B,h).
    \end{equation}
    Finally, we have to check the following two equalities
    \begin{align*}
        \phi(\psi(p))&\equiv p\quad(p\colon A\oplus B)\\
        \psi(\phi(s))&\equiv s\quad(s\colon A+B)
    \end{align*}
    ...
    \end{itemize}
    Conversely, for $s\colon A+B$, we check $\psi(\phi(s))\equiv s$ by induction on the coproduct:
    \begin{itemize}
...
\begin{lpar}{Natural numbers.}
    The type $\N\colon\univ$ is introduced with two constructors: the element \textsl{zero} $0\colon\N$ and a successor operator $\fun{succ}\colon\N\to\N$.

    Predictably, we adopt the conventional notation $1\defeq\fun{succ}(0)$, $2\defeq\fun{succ}(1)$, etc.

    The recursor
    \begin{align*}
        \fun{rec}_\N\colon\prod_{C:\univ}C\to(\N\to C\to C)
            &\to\N\to C
    \end{align*}
    is specified by
    \begin{align*}
    ...
    \begin{equation}\label{tik:natural-rec}
        \begin{tikzcd}[row sep=large, column sep=huge]
            \fun1
                ...
                &\N
                    \arrow[r,"\fun{succ}"]
                    \arrow[d,"{(\fun{id},\fun{rec})}" description]
                &\N
                    \arrow[d,"{(\fun{id},\fun{rec})}"]\\
            \fun1\times C
                    \arrow[r,"0\times c_0"']
                &\N\times C
...
\begin{lpar}{Induction principle.}\label{lpar:induction-principle}
    Assume as given a family $C\colon\N\to\univ$, an element $c_0\colon C(0)$, and a function $c_s\colon\prod_{n:\N} C(n) \to C(\fun{succ}(n))$; then we can construct a dependent function $f\colon\prod_{n:\N} C(n)$ with the defining equations:
    \begin{align*}
        f(0) &\defeq c_0,\\
    ...
    \[
        \fun{ind}_\N:\prod_{C:\N\to\univ}
            C(0) \to \Big(\prod_{n:\N}C(n) \to C(\fun{succ}(n))\Big)
            \to \prod_{n:\N}C(n)
    \]
    plus the equations
    \begin{align*}
...
\begin{lpar}{Blocked computation.}\label{lpar:blocked-computation}
    ...
    \begin{align*}
        1+n &\equiv \fun{succ}(0)+n\\
            ...
            &\equiv \fun{succ}(n).
    \end{align*}
    However, the equation $n =_\N n+0$ does not hold definitionally for an arbitrary variable $n$. It is a proposition that must be proven as a theorem.
...
\begin{lpar}{Propositional Logic.}\label{lpar:propositional-logic}
    ...
    \[
        \begin{tabular}{l|ll}
            \hline
            &\rule{0mm}{4mm}\textbf{Constructive proposition}
                &\textbf{Type Theory's type}\\
            \hline
    ...
    \begin{enumerate}[$1.$]
        ...
        \item A proof $x$ of $A\times B$ consists of a proof $\pi_1(x)$ of $A$ and a proof $\pi_2(x)$ of $B$. Conversely, if $a$ is a proof of $A$ and $b$ is a proof of $B$, then the pair $(a,b)$ is a proof of $A\times B$.

        \item A proof of $A+B$ is constructed either as $\fun{inl}(a)$, given a proof $a$ of $A$, or as $\fun{inr}(b)$, given a proof $b$ of $B$.
...
\begin{lpar}{Predicate logic.}\label{lpar:predicate-logic}
    ...
    \[
        \begin{tabular}{l|ll}
            \hline
            &\rule{0mm}{4mm}\textbf{Predicate logic}
                &\textbf{Type Theory's type}\\[0.3mm]
            \hline
    ...
    \[
        \Big(\prod_{x:A}(P(x)\times Q(x))\Big)
            \to\Big(\prod_{x:A}P(x)\Big)\times\Big(\prod_{x:A}Q(x)\Big).
    \]
    Its proof consists of constructing a witness $f$ for this type.

    Take a dependent function $r$ in the domain. We must construct a pair of dependent functions $(p,q)$, where $p\colon \prod_{x:A}P(x)$ and $q\colon \prod_{x:A}Q(x)$.
    ...
    \[
        f(r) \defeq \big(\lambda x.\,\pi_1(r(x)),\; \lambda x.\,\pi_2(r(x))\big).
    \]
...
In this section we go in further detail about identity family types. Concretely, if $A\colon\univ$, the $\Id_A\colon A\to A\to\univ$ is the type family dependent on two copies of $A$, whose notation is 
\[
...
\[
    \fun{refl}\colon\prod_{a:A}a\eq Aa.
\]
...
\begin{lpar}{Path recursion.}
    ...
    \[
        \fun{rec}_{\eq A}\colon\prod_{B:\univ}B
            \to\prod_{y:A}(x\eq Ay)\to B.
    \]
    It satisfies the computation rule:
    \[
        \fun{rec}_{=_A}(B,c_{\fun{refl}},x,\fun{refl}_x)\equiv c_{\fun{refl}},
    \]
    ...
    \begin{align*}
        f &\colon \prod_{y:A} (x=_A y) \to A \\
        f &\defeq \lambda y.\,\lambda p.\,x,
    \end{align*}
    which is formally defined as $f\defeq\fun{rec}_{=_A}(A, x)$, and satisfies $f(x,p)\eq Ax$ for any path $p$ with origin in $x$.\footnote{Compare this with the function $g\defeq\lambda x.\,\lambda p.\,x$ which satisfies $g(x)\equiv x$ for $x\colon A$.}
\end{lpar}

\begin{lpar}{Comparison with natural recursion.}
    The following table compares based path with natural recursion:
    \small
    \[
        \begin{tabular}{l|l|l}
                & $\N$
                & $\displaystyle\sum_{y:A} x=y$ \\ [4mm]
            \hline\rule{0mm}{5mm}
            Constructors
                & - $0 \colon \N$
                & - $\fun{refl}_x \colon x=_A x$ \\
                & - $\fun{succ} \colon \N \to \N$
                & \\ [1mm]
            \hline\rule{0mm}{4mm}
            Input Data
                & - Base value $c_0 \colon B$
                & - Base value $c_{\fun{refl}} \colon B$ \\
                & - Step $c_s \colon B \to B$
                & \\ [1mm]
            \hline\rule{0mm}{4mm}
            Equations
                & - $f(0) \equiv c_0$
                & - $f(x, \fun{refl}_x) \equiv c_{\fun{refl}}$ \\
                & - $f(\fun{succ}(n)) \equiv c_s(f(n))$
        \end{tabular}
    \]
\end{lpar}
...
\begin{lpar}{General path induction.}
    ...
    \[
        \Big(\sum_{x,y:A}x=_A y\Big)\to\univ.
    \]
    In Type Theory, however, we specify this map using its Curried\footnote{See \nref{lpar:generalized-currying}.} form:
    \[
        C\colon\prod_{x,y:A}(x\eq A y)\to\univ.
    \]
    This distinction explains why we write \/ $C(x,y,p)$ rather than \/ $C((x,y,p))$.

    Given such a motive \/ $C$ and a base case function \/ $c\colon\prod_{x:A}C(x,x,\fun{refl}_x)$, the principle establishes the existence of a dependent function
    \[
        f\colon\prod_{x,y:A}\;\prod_{p:x\eq A y}C(x,y,p)
    \]
    ...
    \begin{equation}\label{eq:ind=A}
        \fun{ind}_{\eq A}\colon\prod_{C:\prod_{x,y:A}(x\eq A y)\to\univ}
            \;\prod_{x:A}C(x,x,\fun{refl}_x)
                \to\prod_{x,y:A}\;\prod_{p:x\eq Ay}C(x,y,p)
    \end{equation}
...
\begin{lpar}{Path symmetry.}
    ...
    \begin{proof}
        Given a type $A$, define the \textsl{motive}\footnote{The type family whose role is to identify the theorem we are about to prove.} family
        \begin{align*}
            C&\colon\prod_{x,y:A}(x\eq Ay)\to\univ\\
            C(x,y,p)&\defeq(y\eq Ax).
        \end{align*}
        For the base case, we need a function of type $\prod_{x:A}C(x,x,\fun{refl}_x)$. Since
        \[
...
\begin{lpar}{Path concatenation.}\label{lpar:path-concatenation}
    ...
    \begin{proof}
        Let $D\colon\prod_{x,y:A}(x\eq Ay)\to\univ$ be the motive family 
        \[
            D(x,y,p) \defeq \prod_{z:A}\;\prod_{q:y\eq Az} (x\eq Az).
        \]
        Since $x\eq Az$ does not depend on the path $q\colon y\eq Az$, we can rewrite $D$ as
        \[
            D(x,y,p) \defeq \prod_{z:A}(y\eq Az)\to(x\eq Az).
        \]
        For the base case we take
        \[
            d(x)\colon D(x,x,\fun{refl}_x)
                \equiv\prod_{z:A}\;(x\eq Az)\to (x\eq Az),\quad d(x)\defeq\lambda z.\,\lambda q.\,q.
        \]
        By path induction, we obtain
        \[
            c(x,y,p)\defeq\fun{ind}_{\eq A}(D,d,x,y,p)\colon D(x,y,p).
        \]
        We can define the concatenation $p\ct q$ by applying this function:
        \[
            p\ct q\defeq c(x,y,p,z,q).
        \]
        The computation rule for path induction implies
        \[
            c(x,x,\fun{refl}_x)\equiv d(x).
        \]
        Therefore,
        \[
            \fun{refl}_x\ct q\equiv c(x,x,\fun{refl}_x,z,q)
...
\begin{lpar}{Right unit law.}\label{lpar:right-unit-law}
    The $\fun{refl}$ function also acts as a right unit for concatenation, except that this time what we get is a propositional equality.
    \begin{lem}
        For every type\/ $A$, every\/ $x,y:A$ and every path\/ $p:x\eq Ay$, we have a path
        \[
    ...
    \begin{proof}
        We perform path induction on $p$. Let $C(x,y,p)$ be the motive family
        \begin{align*}
            C&\defeq\prod_{x,y:A}\;\prod_{p:x\eq Ay}
...
\begin{lpar}{Substitution.}\label{lpar:substitution}
    The principle of \textsl{indiscernibility of identicals} establishes that the equality constructor is compatible with type substitution.\footnote{See also Exercise~\ref{exr:ap-application}.}
        
\begin{lem}
    Let\/ $P\colon A\to\univ$ be a family of types. For any path\/  $e\colon x\eq A y$, there is a function
    \[
        \fun{subst}_P(e)\colon P(x)\to P(y).
    \]
    Moreover, $\fun{subst}_P(\fun{refl}_x)\equiv\lambda z.\,z$.
\end{lem}

\begin{proof}
    Consider the motive $C\colon\prod_{x,y:A}(x\eq Ay)\to\univ$
    \[
    ...
    \[
        \fun{subst}_P(x,y,e)\defeq\fun{ind}_{\eq A}(C,c,x,y,e).
    \]
\end{proof}    

\end{lpar}

\begin{lpar}{Based path induction.}
    ...
    \[
        D\colon\prod_{x:A}(a\eq Ax)\to\univ
    \]
    and an element $d\colon D(a,\fun{refl}_a)$ and obtain a dependent function
    \[
        f\colon\prod_{x:A}\;\prod_{p:a\eq Ax}D(x,p).
    \]
    ...
    \begin{equation}\label{eq:ind'=A}
        \fun{ind}'_{\eq A}\colon\prod_{a:A}
            \;\; \prod_{D:\prod_{x:A} (a\eq A x)\to\univ}
        D(a,\fun{refl}_a)\to\prod_{x:A}\;\prod_{p:a\eq Ax}D(x,p)
    \end{equation}
    with the equality
    \[
        \fun{ind}'_{=_A}(a,D,d,a,\fun{refl}_a)\defeq d.
    \]
    \textbf{Note.} The difference between both forms of path induction is that in the based one, we are free to choose and fix an element $a\colon A$ and then (1)~formulate a property $D(x,p)$ for every $p\colon a\eq Ax$, and (2)~exhibit a witness of $D(a,\fun{refl}_a)$. In general path induction, we formulate $C(x,y,p)$ for generic $p\colon x\eq Ay$, and then prove $C(x,x,\fun{refl}_x)$, also for a generic $x\colon A$. While the general approach may look natural in some special cases, the based one is often simpler, as the generalization $D(x,p)$ that it requires is closer to the concrete base case $D(a,\fun{refl}_a)$, where $a$ is fixed, than the full generalization $C(x,y,p)$ required by the $\fun{ind}$ function. As we will see next, both notions are equivalent.
\end{lpar}
...
\begin{lpar}{Path induction equivalence.}
    ...
    \[
        D\colon\prod_{x:A}(a\eq Ax)\to\univ, \quad \text{and} \quad d\colon D(a,\fun{refl}_a).
    \]
    In order to derive  $D$ using  $\fun{ind}$ we define the motive  $C$ as
    \begin{equation}\label{eq:ind-domain-function}
        C(u,v,q)\defeq\prod_{s:a\eq Au}D(u,s)\to D(v,s\ct q).
    \end{equation}
    For the base case  $c(u)$ we need a term of type
    \[
        \prod_{s:a\eq Au}D(u,s)\to D(u,s\ct\fun{refl}_u).
    \]
    By \nref{lpar:right-unit-law}, there is a path  $\fun{unit}_r(s)\colon s\ct\fun{refl}_u\eq As$. Then, its inverse path satisfies  $\fun{unit}_r(s)^{-1}\colon s\eq As\ct\fun{refl}_u$.
    
    Since we need to turn a  $D(u,s)$ into a  $D(u,s\ct\fun{refl}_u)$, we use the helper function  $\fun{subst}$ on the family  $\lambda t.\,D(u,t)$
    \begin{equation}\label{eq:ind-base-case-subst}
        c(u)\defeq\lambda s.\,\fun{subst}(\fun{unit}_r(s)^{-1}).
    \end{equation}
    By path induction, we obtain
    \[
        E(u,v,q) \defeq\fun{ind}_{\eq A}(C,c,u,v,q)\colon C(u,v,q).
    \]
    This gives us a witness for the based induction:
    \[
        f(x,p)\defeq E(a,x,p)(\fun{refl}_a)(d).
    \]
    This is well-typed because, according to \eqref{eq:ind-domain-function},  $E(a,x,p)(\fun{refl}_a)$ provides a map  $D(a,\fun{refl}_a)\to D(x,\fun{refl}_a\ct p)$, and since  $\fun{refl}_a\ct p\equiv p$, the codomain is precisely  $D(x,p)$. Finally, we apply this map to $d\colon D(a,\fun{refl}_a)$ to get an element of $D(x,p)$.
    ...
    \begin{align*}
        c(a)(\fun{refl}_a)
            &\equiv\fun{subst}(\fun{unit}_r(\fun{refl}_a)^{-1})
                &&;\ \eqref{eq:ind-base-case-subst}\\
            &\equiv\fun{subst}(\fun{refl}_a^{-1})
                &&;\ \fun{unit}_r(\fun{refl}_a)\equiv\fun{refl}_a\\
            &\equiv\fun{subst}(\fun{refl}_a)
...
\begin{lpar}{Additive identity.}\label{lpar:additive-identity}
    Since our definition of addition in $\N$ is by induction in the fist operand, we know that $0+n\equiv n$ for $n\colon\N$. What remains to be seen is the following
    ...
    \begin{itemize}
        \item \textbf{Base case ($n\defeq0$):} We must show $0 =_\N 0 + 0$. Since $0+0\equiv0$, this holds by reflexivity.
        \item \textbf{Inductive step:} We assume the hypothesis $h\colon n =_\N n+0$ and must show $\fun{succ}(n) =_\N \fun{succ}(n)+0$.
        By the computation rules, the \rhs\ reduces: $\fun{succ}(n)+0 \equiv \fun{succ}(n+0)$.
        Thus, the goal becomes $\fun{succ}(n) =_\N \fun{succ}(n+0)$.
        ...
        However, we can also use \nref{lpar:substitution} as follows. Consider the type family $P\colon\N\to\univ$ defined by
        \[
            P\defeq\lambda k.\,\fun{succ}(n)\eq\N\fun{succ}(k).
        \]
        Since $\fun{refl}_{\fun{succ}(n)}\colon P(n)$, we can evaluate 
        \[
            \fun{subst}_P(h)\colon P(n)\to P(n+0)
        \]
...
\begin{lpar}{Associativity of addition.}\label{lpar:associativity-of-addition}
    ...
    \[
        \fun{assoc}\colon\prod_{i,j,k:\N}i+(j+k)\eq\N(i+j)+k.
    \]
    We proceed by induction on $i$. The type family $C\colon\N\to\univ$ is given by:
    \[
        C(i) \defeq \prod_{j,k:\N} i+(j+k)\eq\N(i+j)+k.
    \]
    To define the function $\fun{assoc} \defeq \fun{ind}_\N(C, c_0, c_s)$, we specify:
    \begin{align*}
        c_0 &\colon C(0),\\
        c_s &\colon \prod_{i:\N} C(i) \to C(\fun{succ}(i)).
    \end{align*}
    For the base case $c_0$, we need a witness of $0+(j+k)\eq\N(0+j)+k$. From the definition of addition we know that $0+n \equiv n$. Therefore, the equality reduces to $j+k = j+k$, and we can define the witness by reflexivity:
    \[
    ...
    \]
    where $h(i,j)\colon i+(j+k)\eq\N(i+j)+k$.
    ...
    \begin{align*}
        \fun{succ}(i)+(j+k) &\equiv \fun{succ}(i+(j+k))\\
            ...
            &\equiv (\fun{succ}(i)+j)+k.
    \end{align*}
    The equality in the second line is justified by applying the function $\fun{succ}$ to the path provided by the hypothesis $h(j,k)$. This relies on the \textit{principle of congruence} ($\fun{ap}$), which we prove in Exercise~\ref{exr:ap-application}.
\end{lpar}
...
\begin{lpar}{The order relation on $\N$.}
    ...
    \[
        n\le m \defeq \sum_{k:\N} (n+k=_\N m).
    \]
    ...
    \begin{proof}[Proof sketch]${}$
        \begin{enumerate}[i)]
            \item Take $k \defeq n$. By the definition of addition (see \nref{lpar:natural-addition}), we have the definitional equality $0+n \equiv n$. Thus, $\fun{refl}_n$ has type $0+n =_\N n$. The proof term is $(n,\fun{refl}_n)$.

            \item This relies on the equality $\fun{succ}(n)=n+1$, which can be proved by induction following a similar approach to the one we sketched in \nref{lpar:additive-identity} for proving $n+0=n$ .

            \item This is because $n+0=n$.
            ...
            \end{align*}
            All of this can be easily expressed and proved formally. For instance, the first theorem follows by induction on $n$. The base case is trivial since $0+s=s$. The inductive step consists in showing a witness of
            \[
            ...
            To prove this last theorem, we need to define the predecessor function $\fun{pred}\colon\N\to\N$, which we can easily do by specifying
            \begin{align*}
                \fun{pred}(0)&\defeq0,\\
            ...
            \]
            is inhabited, and the result is a consequence of the transitivity of the equality, a theorem that we will prove in section \ref{sec:identity-types}~\nameref{sec:identity-types}.
        \end{enumerate}
    \end{proof}
\end{lpar}
...
\begin{exr}
    ...
    \[
        \fun{iter} \colon \prod_{C:\univ} C \to (C \to C) \to \N \to C
    \]
...
\begin{solution}
    ...
    \[
        \fun{rec}\colon\prod_{C:\univ}C\to(\N\to C\to C)\to\N\to C
    \]
    ...
    \end{align*}
    Let $p_0\defeq(0,0)$ and $p_s(n,m)\defeq(\fun{succ}(n),n)$. Using the projections can write $\fun q(n)\equiv(q_1(n),q_2(n))$, where $q_1,q_2\colon\N\to\N$. Therefore,
    \begin{align*}
        q_1(0)&\equiv 0\\
        q_2(0)&\equiv 0\\
    ...
    \intertext{and}
        \fun{rec}(C,c_0,c_s,\fun{succ}(n))
            ...
            &\equiv \fun{iter}\big(\hat C,\hat c_0,\hat c_s,
                \fun{succ}(q_1(n))\big)(q_1(q_1(n)))\\
            &\equiv\hat c_s\big(\fun{iter}(\hat C,\hat c_0,\hat c_s,q_1(n))\big)
                (q_1(q_1(n)))\\
            &\equiv c_s\big(q_1(n)\big)\big(\fun{iter}(\hat C,\hat c_0,\hat c_s,q_1(n))\big)
                (q_2(q_1(q_1(n))))\\
            &\equiv c_s\big(q_1(n)\big)
                \big(\fun{rec}(C,c_0,c_s,q_1(n))\big)\\
            &\equiv c_s\big(q_1(n),\fun{rec}(C,c_0,c_s,q_1(n))\big).
    \end{align*}
    Thus, the question of proving that $\fun{rec}$ is propositionally equal to the recursion principle $\fun{rec}_\N$ for $\N$, would follow from the fact that a function $f\colon\N\to\N$ that satisfies $f(0)\equiv0$ and $f(\fun{succ}(n))\equiv\fun{succ}(n)$ is propositionally equal to the identity, i.e., $f=_{\N\to\N}\id_\N$.
    ...
    \[
        \prod_{n:\N} q_1(n) =_\N n.
    \]
    To construct it, we use the induction principle \nref{lpar:induction} for the type family
    \[
        C:\N \to \univ, \quad C(n) \defeq q_1(n) =_\N n,
    \]
    which requires a constant $c_0\colon C(0)$ and a step function 
    \[
        c_s\colon\prod_{n:\N}C(n)\to C(\fun{succ}(n)).
    \]
    The induction principle yields
    \[
        \fun{ind}_\N(C,c_0,c_s)\colon\prod_{n:\N}C(n).
    \]
    For $c_0$, we define it as $\fun{refl}_0$ because $q_1(0) \equiv 0$. For the inductive step, given $n:\N$ and a hypothesis $h:C(n)$ (i.e., $h\colon q_1(n) =_\N n$), we define $c_s(n, h)$ by applying $\fun{succ}$ to $h$ via congruence. This provides a witness for $q_1(\fun{succ}(n))=_\N\fun{succ}(n)$, since $q_1(\fun{succ}(n))\equiv\fun{succ}(q_1(n))$.
    
\end{solution}


\begin{exr}
    Show that if we define\/ $A+B\defeq\sum_{x:\fun2}\fun{rec}_{\fun2}(\univ,A,B,x)$, then we can give a definition of\/ $\fun{ind}_{A+B}$ for which the definitional equalities stated in {\upshape\nref{lpar:coproduct-induction}} hold.
\end{exr}
...
\begin{solution}
    ...
    \[
        \fun{ind}_{A\oplus B}\colon\prod_{C:A\oplus B\to\univ}\;
            \prod_{x:A}C(\fun{in}_A(x))
            \to
            \prod_{y:B}C(\fun{in}_B(y))
            \to
            \prod_{s:A\oplus B}C(s).
    \]
    Fix $C\colon A\oplus B\to\univ$. Given two dependent functions
    \[
        g_0\colon\prod_{x:A}C(\fun{in}_A(x))
        \quad\text{and}\quad
        g_1\colon\prod_{y:B}C(\fun{in}_B(y)),
    \]
    we have to define a function with signature
    \[
        f\colon\prod_{s:A\oplus B} C(s).
    \]
    ...
    \[
        g\colon
            \prod_{t:\fun2\vphantom{\univ y}}\;
            \prod_{z:\fun{rec}_{\fun2}(\univ,A,B,t)}
            C((t,z)).
    \]
    As a result, we will obtain
    \[
        f \defeq\fun{ind}_{\sum_{t:\fun2}\fun{rec}_{\fun2}(\univ,A,B,t)}
            (C,g).
    \]
    ...
    \[
        D(t)\defeq\prod_{z:\fun{rec}_{\fun2}(\univ,A,B,t)}C((t,z)),
    \]
...
\begin{exr}${}$
    \begin{enumerate}[a),font=\upshape]
        ...
        \item Verify that $m^{n+n'}=_\N m^n\cdot m^{n'}$.
    \end{enumerate}
\end{exr}
...
\begin{solution}
    \begin{enumerate}[a)]
        ...
        \begin{enumerate}[1.]
            ...
            \[
                D(i)\defeq\prod_{j,k:\N}i\cdot(j+k)\eq\N i\cdot j+i\cdot k.
            \]
            Then
            \[
                D(0)\equiv\prod_{j,k:\N}0\cdot(i+j)\eq\N0\cdot i+0\cdot j,
            \]
            which is judgmentally equal to $\prod_{i,j:\N}0\eq\N0$ because $0\equiv0+0$. Thus, the constant map $d_0\colon\N\times\N\to\fun{refl}_0$ is a witness.
            ...
            \[
                C(i)\defeq\prod_{j:\N}i\cdot j\eq\N j\cdot i.
            \]
            ...
            \[
                A(i)\defeq\prod_{j,k:\N}(i\cdot j)\cdot k \eq\N i\cdot(j\cdot k).
            \]
            The base case is
            \[
                A(0)\equiv\prod_{j,k:\N}
        ...
        \[
            E\defeq\prod_{n,m,n':\N}m^{n+n'}\eq\N m^n\cdot m^{n'}.
        \]
...
\begin{exr}
    Define the type family\/ $\fun{Fin}\colon\N\to\mathcal{U}$ mentioned at the end of {\upshape\nref{lpar:type-families}}, and the dependent function\/ $\fun{fmax}\colon\prod_{n:\N}\fun{Fin}(\fun{succ}(n))$ mentioned in {\upshape\nref{lpar:Pi-types}}.
\end{exr}
...
\begin{solution}
    ...
    \begin{align*}
        \fun{ack}(\fun{succ}(m), 0)
            ...
            &\equiv \big(\lambda g.\,\fun{rec}_\N(\N, g(1),d_s)\big)(\fun{ack}(m))(0)
    ...
    \begin{align*}
        \fun{ack}(\fun{succ}(m),\fun{succ}(n))
            ...
            &\equiv d_s\big(n,\fun{rec}_\N(\N,\fun{ack}(m)(1),d_s,n)\big)\\
            &\equiv d_s\big(n,c_s(m,\fun{ack}(m))(n)\big)
                &&\text{; \eqref{eq:cs-from-ds}}
    \intertext{\small(at this point the variable $g$ of \eqref{eq:ds-definition} is bound to $\fun{ack}(m)$; consequently)}
            &\equiv d_s\big(n,\fun{ack}(\fun{succ}(m),n)\big)
                &&\text{; recursion}\\
            &\equiv\fun{ack}(m)\big(\fun{ack}(\fun{succ}(m),n)\big)
...
\begin{solution}
    ...
    \[
        \big(P+(P\to\fun0)\to\fun0\big)\to\fun0.
    \]
    More generally, let us define a function
    \[
        f\colon\big(P+(P\to Z)\to Z\big)\to Z.
    \]
...
\begin{exr}
    ...
    \[
        f\colon\prod_{x:A}\;\prod_{p:x=x} (p=\fun{refl}_x)
    \]
...
\begin{solution}
    ...
    The given expression only considers paths of the form $p: x=x$, between $x$ and itself, while the induction principle requires paths between two variables.
\end{solution}
...
\begin{solution}
    ...
    \[
        \prod_{i,j:\N} i+j =_\N j+i.
    \]
    We proceed by induction on $i$. Define the motive $C \colon \N \to \univ$ by
    \[
        C(i) \defeq \prod_{j:\N} i+j =_\N j+i.
    \]
    
    \textbf{Base case:} We must provide a term $c_0 \colon \prod_{j:\N} 0+j =_\N j+0$.
    By definition of addition, $0+j \equiv j$. Thus, the goal reduces to showing $j =_\N j+0$. We have already constructed a witness for this in \nref{lpar:additive-identity}.

    \textbf{Inductive step:} Let $i:\N$ and assume the inductive hypothesis
    \[
        h \colon \prod_{j:\N} i+j =_\N j+i.
    \]
    We must show that for any $j$, $\fun{succ}(i)+j =_\N j+\fun{succ}(i)$.
    \begin{align*}
        \fun{succ}(i)+j &\equiv \fun{succ}(i+j) \\
            &=_\N \fun{succ}(j+i) 
                &&\text{; $h(j)$ and congruence}\\
            &=_\N (j+i)+1 
                &&\text{; } \fun{succ}(n) =_\N n+1\\
            &=_\N j+(i+1) 
                &&\text{; associativity}\\
            &=_\N j+\fun{succ}(i) 
                &&\text{; } n+1 =_\N \fun{succ}(n).
    \end{align*}
\end{solution}
...
\begin{solution}
    \begin{enumerate}[a)]
        ...
        \begin{align*}
            C_f&\colon\prod_{x,y:A}(x\eq Ay)\to\univ\\
        ...
        \begin{align*}
            \fun{ind}_{\eq A}(C_f,c_f)\colon
                \prod_{x,y:A}\;\prod_{p:x\eq Ay}f(x)\eq Bf(y),
        \end{align*}
        which satisfies
        \[
        ...
        \begin{align*}
            D_f&\colon\prod_{x,y:A}(x\eq Ay)\to\univ\\
                D_f(x,y,p)&\defeq\prod_{z:A}\;\prod_{q:y\eq Az}
                \tilde f(p\ct q)\eq {f(x)\eq Bf(z)}
                    \tilde f(p)\ct\tilde f(q).
        \end{align*}
        Using pat a) and the fact that $\fun{refl}$ is the  definitionally left concatenation unit, we deduce
        \begin{align*}
            D_f(x,x,\fun{refl}_x)
                &\equiv\prod_{z:A}\;\prod_{q:x\eq Az}
...
\begin{exr}
    ...
    \begin{enumerate}[a), font=\upshape]
        \item Define the negation function\/ $\fun{not} : \fun{2} \to \fun{2}$.
        \item Prove that negation is an involution, i.e., construct a witness for
        \[
            \prod_{b:\fun2}\fun{not}(\fun{not}(b))\eq {\fun2} b.
        \]
    \end{enumerate}
\end{exr}
...
\begin{solution}
    \begin{enumerate}[a)]
        ...
        \[
            C\defeq\prod_{b:\fun2}\fun{not}(\fun{not}(b))\eq{\fun2}b.
        \]
...
\begin{exr}
    Prove that substitution over a constant type family is the identity function. That is, given a type\/ $B$ and the constant family\/ $P(x)\defeq B$, construct a witness for
    \[
        \prod_{x,y:A}\;
            \prod_{p:x\eq Ay}\;
            \prod_{b:B}\fun{subst}_P(p,b)\eq Bb.
    \]
\end{exr}

\begin{solution}
    The motive $C$ used in \nref{lpar:substitution} to derive the transport function $\fun{subst}_P$ is given by
    \[
    ...
    C(x,y,p)\defeq B\to B,
    \]
    while $c\colon C(x,x,\fun{refl}_x)$ remains the identity $c\defeq\lambda b\colon B.\,b$. Let us also recall that $\fun{subst}_P(x,y,p)\equiv\fun{ind}_{\eq A}(C,c,x,y,p)$.
    
    Consider the motive
    \[
    D(x,y,p)\defeq\prod_{b:B}\fun{subst}_P(p,b)\eq B b.
    \]
    For the base case, we need a witness $d$ of type $\prod_{x:A} D(x,x,\fun{refl}_x)$.
    Since
    \[
        \fun{subst}_P(x,x,\fun{refl}_x)\equiv\id_B,
    \]
    and $\id_B(b)\equiv b$, evaluating the motive at the base yields
    \[
    D(x,x,\fun{refl}_x) \equiv \prod_{b:B}b\eq B b.
    \]
    ...
    \[
    \fun{ind}_{\eq A}(D,d)\colon\prod_{x,y:A}\;\prod_{p:x\eq A y} D(x,y,p).
    \]
\end{solution}

\begin{exr}
    Let\/ $f\colon A\to B$ be a function between types\/ $A$ and\/ $B$.
    Show that the action on paths\/ $\fun{ap}_f$ can be defined strictly in terms of the transport function\/ $\fun{subst}$.
    Specifically, given\/ $x,y\colon A$ and a path\/ $p\colon x\eq Ay$, construct a term of type\/ $f(x)\eq Bf(y)$ using only\/ $\fun{subst}$ and\/ $\fun{refl}$.
\end{exr}

\begin{solution}
    Fix\/ $x\colon A$ and consider the family\/ $P\colon A\to\univ$ defined by
    \[
        P(y)\defeq f(x)\eq B f(y).
    \]
    By transport, we know that there is a function
    \[
        \fun{subst}_P\colon\prod_{y:A}\;\prod_{p:x\eq Ay}P(x)\to P(y).
    \]
    Since\/ $\fun{refl}_{f(x)}\colon P(x)$, given a path\/ $e\colon x\eq A y$, we can apply the transport function to obtain
    \[
        \fun{subst}_P(y,e)(\fun{refl}_{f(x)})\colon P(y).
    \]
