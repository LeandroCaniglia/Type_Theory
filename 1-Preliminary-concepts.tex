...
\begin{lpar}{Blocked computation.}\label{lpar:blocked-computation}
    ...
    In our case, $\Phi\defeq n$. Since $n$ is a variable and not a constructor, neither reduction rule applies. The machine cannot simplify the expression further, and the computation \textit{blocks}. Thus, we cannot rely on automatic reduction; we must construct an explicit proof term (deduction).
    ...
    \end{itemize}
    To complete the proof, we need to apply the function $\fun{succ}$ to both sides of the hypothesis $h$. This relies on the \textit{principle of congruence} that states that functions respect equality, a lemma that we will formally introduce in the next chapter (see also \nref{lpar:substitution}).
\end{lpar}
...
\begin{lpar}{Associativity of addition.}\label{lpar:associativity-of-addition}
    ...
    \begin{align*}
        \fun{succ}(i)+(j+k) &\equiv \fun{succ}(i+(j+k))\\
            &\eq\N \fun{succ}((i+j)+k) && \text{; by } h(j,k)\\
            &\equiv \fun{succ}(i+j)+k\\
            &\equiv (\fun{succ}(i)+j)+k.
    \end{align*}
    The equality in the second line is justified by applying the function $\fun{succ}$ to the path provided by the hypothesis $h(j,k)$. This relies on the \textit{principle of congruence} ($\fun{ap}$), which we will prove in Chapter~2. As we will see, the expression to fill the ellipsis above is $\fun{ap}_{\fun{succ}}(h(j,k))$.
\end{lpar}
...
\begin{lpar}{Path recursion.}
    Consider the type
    \[
        E \defeq\sum_{x,y:A}x\eq Ay.
    \]
    We can define the trivial map
    \begin{align*}
        r\colon A&\to E\\
        x&\mapsto(x,x,\fun{refl}_x).
    \end{align*}
\end{lpar}
...
\begin{lpar}{Path concatenation.}\label{lpar:path-concatenation}
    ...
    \begin{lem}\label{lem:path-concatenation}
        ...
        \]
        such that\/ $\mathsf{refl}_x\cdot q\equiv q$ for any\/ $x\colon A$ and $q\colon x\eq Az$.
    \end{lem}
    ...
    \begin{proof}
        ...
        Therefore,
        \[
            \fun{refl}_x\cdot q\equiv c(x,x,\fun{refl}_x,z,q)
...
\begin{lpar}{Right unit law.}\label{lpar:right-unit-law}
    ...
    \begin{lem}
        ...
        \[
            \fun{unit}_r(p)\colon p\cdot\fun{refl}_y\eq{x\eq Ay}p.
        \]
    \end{lem}
    ...
    \begin{proof}
        ...
        \begin{align*}
            C&\defeq\prod_{x,y:A}\;\prod_{p:x\eq Ay}
                (p\cdot\fun{refl}_y\eq{x\eq Ay}p).
        \end{align*}
        For the base case, we assume $y \equiv x$ and $p \equiv \fun{refl}_x$. We must construct a term of type
        \[
            \fun{refl}_x\cdot\fun{refl}_x\eq{x\eq Ax}\fun{refl}_x.
        \]
        By Lemma~\ref{lem:path-concatenation}, we have the definitional equality $\fun{refl}_x \cdot \fun{refl}_x \equiv \fun{refl}_x$. Thus, the goal simplifies to $\fun{refl}_x\eq{x\eq Ax}\fun{refl}_x$, which is inhabited by $c(x)\defeq\fun{refl}_{\fun{refl}_x}$.
...
\begin{lpar}{Substitution.}\label{lpar:substitution}
    The principle of \textsl{indiscernibility of identicals} establishes that the equality constructor is compatible with type substitution. 
...
\begin{lpar}{Path induction equivalence.}
    ...
    \begin{equation}\label{eq:ind-domain-function}
        C(u,v,q)\defeq\prod_{s:a\eq Au}D(u,s)\to D(v,s\cdot q).
    \end{equation}
    For the base case  $c(u)$ we need a term of type
    \[
        \prod_{s:a\eq Au}D(u,s)\to D(u,s\cdot\fun{refl}_u).
    \]
    By \nref{lpar:right-unit-law}, there is a path  $\fun{unit}_r(s)\colon s \cdot \fun{refl}_u\eq As$. Then, its inverse path satisfies  $\fun{unit}_r(s)^{-1}\colon s\eq As\cdot\fun{refl}_u$.
    
    Since we need to turn a  $D(u,s)$ into a  $D(u,s\cdot\fun{refl}_u)$, we use the helper function  $\fun{subst}$ on the family  $\lambda t.\,D(u,t)$
    ...
    \]
    This is well-typed because, according to \eqref{eq:ind-domain-function},  $E(a,x,p)(\fun{refl}_a)$ provides a map  $D(a,\fun{refl}_a)\to D(x,\fun{refl}_a\cdot p)$, and since  $\fun{refl}_a\cdot p\equiv p$, the codomain is precisely  $D(x,p)$. Finally, we apply this map to $d\colon D(a,\fun{refl}_a)$ to get an element of $D(x,p)$.
...
\begin{solution}
    ...
    \end{align*}
    Thus, by \nref{lpar:substitution}, the question of proving that $\fun{rec}$ is propositionally equal to the recursion principle $\fun{rec}_\N$ for $\N$, would follow from the fact that a function $f\colon\N\to\N$ that satisfies $f(0)\equiv0$ and $f(\fun{succ}(n))\equiv\fun{succ}(n)$ is propositionally equal to the identity, i.e., $f=_{\N\to\N}\id_\N$.

    However, strictly speaking, that would be too much for our needs, since the equality $q_1(n)=n$ for $n\colon\N$ would imply by substitution that $\fun{rec}$ satisfies the recursion principle equations propositionally, which is what we want.
    ...
    \]
    For $c_0$, we define it as $\fun{refl}_0$ because $q_1(0) \equiv 0$. For the inductive step, given $n:\N$ and a hypothesis $h:C(n)$ (i.e., $h\colon q_1(n) =_\N n$), we define $c_s(n, h)$ by applying $\fun{succ}$ to $h$ via substitution (see \nref{lpar:substitution}). This provides a witness for $q_1(\fun{succ}(n))=_\N\fun{succ}(n)$, since $q_1(\fun{succ}(n))\equiv\fun{succ}(q_1(n))$.
...
\begin{solution}
    \begin{enumerate}[a)]
        ...
        \begin{enumerate}[1.]
            ...
            For the inductive step, take $p\colon n\cdot0\eq\N0$. We have to see that $\fun{succ}(n)\cdot0\eq\N0$ is inhabited. Since $\fun{succ}(n)\cdot0\equiv n\cdot0+0$, the conclusion follows from path concatenation: take $q\colon n\cdot0+0\eq\N n\cdot0$ and obtain $q\cdot p\colon\fun{succ}(n)\cdot0\eq\N0$.
            ...
            \end{align*}
            where we have used \nref{lpar:substitution} in obvious ways. The conclusion follows from \nref{lpar:path-concatenation}.
...
\begin{solution}
    We have to exhibit a witness of
    \[
        \prod_{i,j:\N}i+j=_\N j+i.
    \]
    Define the motive $C\colon\N\to\univ$ by
    \[
        C(i) \defeq\prod_{j:\N}i+j=_\N j+i
    \]
    By the induction principle on $\N$, to show a witness of $\prod_{i:\N}C(i)$, we have to provide two terms
    \[
        c_0\colon C(0)
        \quad\text{and}\quad
        c_s\colon\prod_{i:\N}C(i)\to C(\fun{succ}(i)).
    \]
    We already know that $0+j\equiv j$ and $j+0=_\N j$ for $j\colon\N$ (see \nref{lpar:blocked-computation}). Thus, the equation $0+j=_\N j+0$ is judgmentally equal to $j=_\N j+0$ and so we have a witness of $C(0)$.

    For the inductive step, if $h\colon i+j=_\N j+i$, we can apply $\fun{succ}$ by substitution and proceed by concatenation as follows
    \begin{align*}
        \fun{succ}(i)+j &\equiv \fun{succ}(i+j)\\
            &=_\N\fun{succ}(j+i) &&\text{; $h$ + subst.}\\
            &=_\N(j+i)+1\\
            &=_\N j+(i+1)  &&\text{; assoc.}\\
            &=_\N j+\fun{succ}(i) &&\text{; subst.}
    \end{align*}
\end{solution}
...
\begin{solution}
    ...
    \textbf{Base case ($i \equiv 0$):} We must provide a term $c_0 \colon \prod_{j:\N} 0+j =_\N j+0$.
