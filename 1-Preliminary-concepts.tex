...
\begin{exr}
    ...
    \begin{align*}
        \fun{iter}(C, c_0, c_s, 0) &:\equiv c_0,\\
        \fun{iter}(C, c_0, c_s, \fun{succ}(n)) &:\equiv c_s(\fun{iter}(C, c_0, c_s, n)),
    \end{align*}
...
\begin{solution}
    ...
    To do this, we will apply $\fun{iter}$ to $\hat C\defeq\N\to C$. For  $\hat c_0$ take the constant function $\lambda n.\,c_0$. For $\hat c_s$ we take
    \begin{align*}
        \hat c_s&\colon(\N\to C) \to (\N\to C)\\
        \hat c_s(\theta)&\defeq\lambda n.\,c_s(n)(\theta(\fun{pred}(n))).
    \end{align*}
    This is well defined: given $n\colon\N$, we have $c_s(n)\colon C\to C$ and $\theta(\fun{pred}(n))\colon C$, and so $c_s(n)(\theta(\fun{pred}(n)))\colon C$.

    Now we can define
    \begin{align*}
        \fun{rec}(C,c_0,c_s,n)
            &\defeq \fun{iter}(\hat C,\hat c_0,\hat c_s,n)(\fun{pred}(n)),
    \end{align*}
    which satisfies
    \begin{align*}
    ...
    \intertext{and}
        \fun{rec}(C,c_0,c_s,\fun{succ}(n))
            &\equiv \fun{iter}(\hat C,\hat c_0,\hat c_s,\fun{succ}(n))(n)\\
            &\equiv \hat c_s(\fun{iter}(\hat C,\hat c_0,\hat c_s,n))(n)\\
            &\equiv c_s(n)(\fun{iter}(\hat C,\hat c_0,\hat c_s,n)
                (\fun{pred}(n)))\\
            &\equiv c_s(n)(\fun{rec}(C,c_0,c_s,n))\\
            &\equiv c_s(n,\fun{rec}(C,c_0,c_s,n)),
    \end{align*}
    as required.
\end{solution}
