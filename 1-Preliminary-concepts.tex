...
\begin{lpar}{General path induction.}
    ...
    \]
    such that $f(x,x,\fun{refl}_x)\defeq c(x)$. In other words, the induction principle for identity types is defined as
    \[
        \fun{ind}_{\eq A}\colon\prod_{C:\prod_{x,y:A}(x\eq A y)\to\univ}
            \;\prod_{x:A}C(x,x,\fun{refl}_x)
                \to\prod_{x,y:A}\;\prod_{x\eq Ay}C(x,y,p)
    \]
    and the equation
    \[
        \fun{ind}_{\eq A}(C,c,x,x,\fun{refl}_x)\defeq c(x).
    \]
\end{lpar}
...
\begin{lpar}{Based path induction.}
    ...
    \]
    \textbf{Note.} The difference between both forms of path induction is that in the based one, we are free to choose and fix an element $a\colon A$ and then (1)~formulate a property $C(x,p)$ for every $p\colon a\eq Ax$, and (2)~exhibit a witness of $C(a,\fun{refl}_a)$. In general path induction, we formulate $C(x,y,p)$ for generic $p\colon x\eq Ay$, and then prove $C(x,x,\fun{refl}_x)$, also for a generic $x\colon A$. While the general approach may look natural in some special cases, the based one is often simpler, as the required generalization is much closer to the concrete base case where $a$ is fixed.
\end{lpar}
