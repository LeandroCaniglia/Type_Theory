...
\begin{proof}
    Given a type $A$ define
    \begin{align*}
        C&\colon\prod_{x,y:A}(x\eq Ay)\to\univ\\
        C&\defeq\lambda x.\,\lambda y.\,\lambda p.\,(y\eq Ax)
    \end{align*}
    and take
    \begin{align*}
        c&\defeq\lambda x.\,\fun{refl}_x\colon C(x,x,\fun{refl}_x).
    \end{align*}
    Then,
    \[
        \fun{ind}_{\eq A}(C,c,x,y,p)\colon y\eq A x
    \]
    and we can define by general path induction $p^{-1}\defeq\fun{ind}_{\eq A}(C,c,x,y,p)$ for every $p\colon(x\eq Ay)$. In addition, we also have
    \[
        \fun{refl}_x^{-1}
