...
\begin{lpar}{Polymorphic functions.}
    ...
    \]
    Note that given $f\colon\prod_{(x:A)}\prod_{(y\colon B(x))}C(x,y)$, we have $f(a,b)\colon C(a,b)$, assuming $a\colon A$ and $b\colon B$.
\end{lpar}
...
\begin{lpar}{Dependent functions over product.}\label{lpar:Pi-types-over-product}
    According to the specification of $\Pi$-types (see \nref{lpar:Pi-types}), given $A,B\colon\univ$ and a type family $C\colon A\times B\to\univ$, a dependent function
    \[
...
\begin{lpar}{Coproduct induction.}
...
\begin{lpar}{The type of booleans.}
    ...
    \end{align*}
    which express the fact that $f\colon\fun2\to C$ is given by a pair of elements of type $C$.
...
\begin{lpar}{Coproducts as Indexed Disjoint Unions.}
...
\begin{lpar}{Induction principle.}\label{lpar:induction-principle}
    ...
    plus the equations
    \begin{align*}
        \fun{ind}_\N(C, c_0, c_s, 0) &:\equiv c_0,\\
        \fun{ind}_\N(C, c_0, c_s, \fun{succ}(n)) &:\equiv c_s(n, \fun{ind}_\N(C, c_0, c_s, n)).
    \end{align*}
...
\begin{lpar}{Blocked computation.}\label{lpar:blocked-computation}
    ...
    \[
        n =_\N n+0
    \]
    does not hold definitionally for an arbitrary variable $n$. It is a proposition that must be proven as a theorem.
...
\begin{lpar}{Propositional Logic.}\label{lpar:propositional-logic}
    ...
    \[
        ...
        \end{tabular}
    \]
    Let's review them one by one, assuming that types $A$ and $B$ represent propositions and their elements represent proofs.
    \begin{enumerate}[$1.$]
...
\begin{lpar}{Predicate logic.}\label{lpar:predicate-logic}
    The following table extends the way we interpret propositions as types (see \nref{lpar:propositional-logic}) to quantifiers
    \[
        \begin{tabular}{l|ll}
            \hline
...
\begin{lpar}{Indiscernibility of identicals.}
    This principle establishes that for every family $C\colon A\to\univ$, there is a function
    \[
        f\colon\prod_{x,y:A}\;\prod_{p:x\eq Ay}C(x)\to C(y)
    \]
    satisfying $f(x,x,\fun{refl}_x)\defeq\id_{C(x)}\colon C(x)\to C(x)$. The interpretation is that a proof of $C(y)$ can be derive from a proof of $C(x)$, provided that $x\eq Ay$.
\end{lpar}
...
\begin{lpar}{Path symmetry.}
    The following lemma formalizes the symmetry of the relation $x\eq Ay$.
...
\begin{lpar}{Path concatenation.}
    ...
    \begin{proof}
        ...
        \[
            c(x,y,p)\defeq\fun{ind}_=(D,d,x,y,p)\colon D(x,y,p).
        \]
...
\begin{lpar}{Right unit law.}
    ...
    \begin{proof}
        ...
        \[
            \fun{unit}_r(p) \defeq \fun{ind}_=(C,c,x,y,p).
        \]
    \end{proof}
...
\begin{lpar}{Based path induction.}
    A different approach to path induction, which we will show is equivalent to the general one, consists in fixing an element $a\colon A$ before considering the type family~$C$, specific for $a$,
    \[
        C\colon\prod_{x:A}(a\eq Ax)\to\univ
    \]
    and an element $c\colon C(a,\fun{refl}_a)$ and obtain a dependent function
    \[
        f\colon\prod_{x:A}\;\prod_{p:a\eq Ax}C(x,p).
    \]
    This means that to define an element of the family $C$ for all $x$ and $p$, it suffices to consider the case where $x$ is $a$ and $p$ is $\fun{refl}_a$.
    
    As a function, based path induction becomes
    \begin{equation}\label{eq:ind'=A}
        \fun{ind}'_{\eq A}\colon\prod_{a:A}
            \;\; \prod_{C:\prod_{x:A} (a\eq A x)\to\univ}
        C(a,\fun{refl}_a)\to\prod_{x:A}\;\prod_{p:a\eq Ax}C(x,p)
    \end{equation}
    with the equality
    \[
        \fun{ind}'_{=_A}(a,C,c,a,\fun{refl}_a)\defeq c.
    \]
    \textbf{Note.} The difference between both forms of path induction is that in the based one, we are free to choose and fix an element $a\colon A$ and then (1)~formulate a property $C(x,p)$ for every $p\colon a\eq Ax$, and (2)~exhibit a witness of $C(a,\fun{refl}_a)$. In general path induction, we formulate $C(x,y,p)$ for generic $p\colon x\eq Ay$, and then prove $C(x,x,\fun{refl}_x)$, also for a generic $x\colon A$. While the general approach may look natural in some special cases, the based one is often simpler, as the generalization $C(x,p)$ that it requires is closer to the concrete base case $C(a,\fun{refl}_a)$, where $a$ is fixed, than the full generalization $C(x,y,p)$ required by the $\fun{ind}$ function.
\end{lpar}
...
\begin{lpar}{Path induction equivalence.}
    ...
    \begin{align*}
        C'&\defeq\lambda y.\,\lambda p'.\,C(x,y,p')\colon\prod_{y:A}(x\eq Ay)\to\univ\\
        c' &\defeq c(x) \colon C(x,x,\fun{refl}_x)\\
        y &\colon A\\
        p &\colon x\eq A y
    \end{align*}
    Thus, the application matches the signature of $\fun{ind}'_{\eq A}$. Moreover,
    \[
        \fun{ind}_{\eq A}(C,c,x,x,\fun{refl}_x)
        \equiv \fun{ind}'_{\eq A}(x, C', c', x, \fun{refl}_x)
        \equiv c(x),
    \]
    For the converse, let's first consider all the type families
    \[
        D\colon\prod_{z:A}(x\eq Az)\to\univ,
    \]
    where $x\colon A$ is given. For every $y\colon A$ and $p\colon x\eq Ay$, we can define
    \[
        \hat D\defeq\lambda x',y\colon A.\,p\colon x'\eq Ay.\,D(y,p)
    \]
    \[
        D(x,\fun{refl}_x)\to D(y,p)
    \]
    
    We can express $\fun{ind}'_{\eq A}$ in terms of $\fun{ind}_{\eq A}$ as follows. Given
    \begin{align*}
        a&\colon A,\\
        C&\colon\prod_{x:A}(a\eq Ax)\to\univ\\
        c&\colon C(a,\fun{refl}_a)\\
        x&\colon A,\\
        p&\colon a\eq Ax,
    \end{align*}
    we define
    \[
        \fun{ind}'_{\eq A}(a,C,c,x,p)
        \defeq\fun{ind}_{\eq A}(\hat C,\hat c,a,x,p),
    \]
    where
    \begin{align*}
        \hat C &\defeq \lambda x,y\colon A.\,\lambda p\colon x\eq Ay.\,
    \Bigg(\prod_{D:\prod_{z:A}(x\eq Az)\to\univ}D(x,\fun{refl}_x)
    \to D(y,p)\Bigg)
    \colon\prod_{x,y:A}(x\eq Ay)\to\univ\\
        \hat c &\defeq \lambda z.\, \fun{ind}_{\eq A}(\hat C,\hat c,a,z, \fun{refl}_z)\colon \prod_{z:A}\hat C(z,z,\fun{refl}_z) \equiv \prod_{z:A}C(z,\fun{refl}_z).
    \end{align*}
    Moreover, at the base case ($x=a$, $p=\fun{refl}_a$),
    \[
        \fun{ind}'_{\eq A}(a,C,c,a,\fun{refl}_a)
          \equiv c
    \]
    And by definition:
    \[
        \fun{ind}_{\eq A}(\hat C,\hat c,a,a,\fun{refl}_a)
          \equiv \hat c(a) \equiv c,
    \]
    thus the definitions coincide at the base case.
\end{lpar}

    Here is the logic for defining $\fun{ind}'$ from $\fun{ind}$.
    \begin{enumerate}
        \item \textbf{The Challenge.}
        \begin{itemize}
            \item We have: The machine $\fun{ind}$ which works for any start point $u$, end point $v$, and path $q$.
            
            \item We want: To solve a problem specifically for a fixed start point $a$, end point $x$, and path $p$.
            
            \item The Mismatch: $\fun{ind}$ demands a base case that works for all start points $u$ (the diagonal). But we only have a base case $d$ for our specific point $a$.
        \end{itemize}
        
        \item \textbf{The Strategy.} \textit{Transporting Evidence}: To use the general machine, we have to trick it. We cannot just ask it to solve our specific problem for $a$. Instead, we ask it to solve a conditional problem:
        \begin{quote}
            \textit{For any start point $u$, if there is a path from $a$ to $u$, can we transport our evidence from $a$ to $u$?}
        \end{quote}
         We define a motive $C(u, v, q)$ that carries the \textit{history} of how we got to~$u$.
         
        \item \textbf{The Definition.} Let's define $\fun{ind}'(a,D,d,x,p)$ using $\fun{ind}$.
        
        \textbf{Step 1:} \textit{Define the Motive $C$}: We define $C(u, v, q)$ to be a function type:
        \[
            C(u, v, q) \defeq \prod_{k: a=u} D(u, k) \to D(v, k \cdot q),
        \]
        which says that, given a path $k$ from $a$ to $u$, and a proof that $D$ holds at the end of $k$, we obtain a proof that $D$ holds at the end of the extended path $k \cdot q$.\footnote{Note: $k \cdot q$ is path concatenation/transitivity.}
        
        \textbf{Step 2:} \textit{Prove the Base Case for $\fun{ind}$}: We need a witness $c(u)$ for the diagonal $C(u, u, \fun{refl}_u)$.
        \[
            c(u) : \prod_{k: a=u} D(u, k) \to D(u, k \cdot \fun{refl}_u).
        \]
        Since $k \cdot \fun{refl}_u \equiv k$, this simplifies to:
        \[
            D(u, k) \to D(u, k).
        \]
        This is just the Identity Function $\lambda z.z$. So, $\fun{ind}$ accepts this.
        
        \textbf{Step 3:} \textit{Run the Machine}: We apply ind to this motive and base case. It gives us a term $F$ that works for any $u, v, q$:
        \[
            F(u,v,q)\colon\prod_{k:a=u}D(u,k)\to D(v,k\cdot q).
        \]
        
        \textbf{Step 4:} \textit{Instantiate for our Specific Problem}: Now we use $F$ to solve our specific problem $\fun{ind}'(a,D,d,x,p)$. We set:
        \[
            u \defeq a,\quad v \defeq x,\quad
            q \defeq p,\quad k \defeq \fun{refl}_a.
        \]
        Substituting these into $F$, we get a function of type:
        \[
            D(a,\fun{refl}_a)\to D(x,\fun{refl}_a\cdot p).
        \]
        Since $\fun{refl}_a \cdot p \equiv p$, this is:
        \[
            D(a,\fun{refl}_a)\to D(x,p).
        \]
        
        \textbf{Step 5:} \textit{Apply the Witness}: We simply apply this function to our specific witness $d$, which has type $D(a,\fun{refl}_a)$:
        \[
            \text{Result} \defeq F(a,x,p)(\fun{refl}_a)(d).
        \]
    \end{enumerate}

    \textbf{Summary:} To define $\fun{ind}'$ from $\fun{ind}$, we use ind to prove a \textit{transitivity/transport} law:
    \begin{quote}
        \textit{If property $D$ holds for a path ending at $u$, it holds for any extension of that path.}
    \end{quote}
    Then we simply apply that law to the trivial path $\fun{refl}_a$ to start the chain.
