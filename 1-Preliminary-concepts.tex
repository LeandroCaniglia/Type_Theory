...
\begin{lpar}{Natural numbers.}
    ...
    \end{align*}
    We can represent this in a definitional commutative diagram
...
\begin{lpar}{Associativity of addition.}\label{lpar:associativity-of-addition}
    ...
    \]
    were $h(i,j)\colon i+(j+k)\eq\N(i+j)+k$.
...
\begin{solution}
    ...
    \]
    and an element $p_0\colon\N\times\N$. Invoking $\fun{iter}$, with these two pieces of data we obtain a function $\fun q\colon\N\to(\N\times\N)$, given by $\defeq\lambda n.\,\fun{iter}(\N\times\N,p_0,p_s,n)$ which satisfies
    \begin{align*}
        \fun q(0)&\defeq p_0\\
...
\begin{solution}
    \begin{enumerate}[a)]
        ...
        \item From \nref{lpar:blocked-computation} and [\nref{lpar:associativity-of-addition}], we know that $0$ is additive identity of $\N$ and that addition is associative. Therefore, we have to prove the following propositions
        \begin{enumerate}[1.]
            ...
            \]
            which is judgmentally equal to $\prod_{i,j:\N}0\eq\N0$ because $0\equiv0+0$. Thus, the constant map $d_0\colon\N\times\N\to\fun{reflx}_0$ is a witness.

            For the inductive step take $p\colon D(i)$ and observe that
            \begin{align*}
                \fun{suc}(i)\cdot(j+k) &\equiv i\cdot(j+k)+(j+k)\\
            ...
            \end{align*}
            where we have used \nref{lpar:substitution} in obvious ways. Te conclusion follows from \nref{lpar:path-concatenation}.
        ...
        \item We have to construct a function
        \[
            \fun{expAdd}\colon\prod_{m,n,n':\N}m^{n+n'}\eq\N m^n\cdot m^{n'}.
        \]
        To derive $\fun{expAdd}$ from the induction principle, we introduce the motive $E\colon\N\to\univ$ given by
        \[
            E(n)\defeq\prod_{m,n':\N}m^{n+n'}\eq\N m^n\cdot m^{n'}.
        \]
        ...
        \end{align*}
        Suppose that $p\colon E(n)$. Fix $m,n'\colon\N$ and put
        \[
            q\defeq p(m,n')\colon m^{n+n'}\eq\N m^n\cdot m^{n'}.
        \]
        We have to construct a path for
        \[
            m^{\fun{succ}(n)+n'}\eq\N n^{\fun{succ}(n)}\cdot m^{n'}.
        \]
        Now
        \begin{align*}
