...
\begin{lpar}{Dependent functions \textrm{(a.k.a.~$\Pi$-types)}.} \label{lpar:Pi-types}
    ...
    \[
        \prod_{x:\,A}A\to A\quad\text{stands for}\quad\prod_{x:\,A}(A\to A).
    \]
\end{lpar}
...
\begin{lpar}{Polymorphic functions.}
    ...
    \[
        \fun{swap}\colon
            \prod_{A,B,C:\,\univ}(A\to B\to C)\to(B\to A\to C)
    \]
    defined as
    \[
...
\begin{lpar}{Products revisited.}\label{lpar:products-revisited}
    ...
    \begin{enumerate}[i),font=\scshape]
        ...
        \[
            \fun{rec}_{\fun1}\colon\prod_{C:\,\univ}C\to\fun1\to C
        \]
        specified as
        \[
...
\begin{lpar}{Dependent functions over product.}\label{lpar:Pi-types-over-product}
    ...
    \[
        g\colon\prod_{x:\,A}\;\prod_{y:\,B}C((x,y))
    \]
    as
    \[
        f((x,y))\defeq g(x)(y).
...
\begin{lpar}{Recursion in $\Sigma$-types.}\label{lpar:Sigma-type-rec-and-ind}
    ...
    \textbf{Interpretation.} To prove $C(p)$ for every $p\colon\sum_{x:\,A}B(x)$ it suffices to show a proof of $C((a,b))$ for every canonical pair $(a,b)$.
\end{lpar}
...
\begin{lpar}{Relations.}\label{lpar:relations}
    ...
    For instance, let $R(2,n)$ stand for the type representing the relation ``$2\mid n$'', where $n\colon\N$. The elements of the type $\sum_{n:\,\N}R(2,n)$ represent the even numbers paired with the evidence of their evenness. Thus, if $\omega$ is a witness that $2\cdot3\eq\N6$, we have
    \[
...
\begin{lpar}{Induction principle.}\label{lpar:induction-principle}
    ...
    \[
        \fun{ind}_\N\colon\prod_{C:\,\N\to\univ}
...
\begin{lpar}{Path recursion.}
    ...
    \begin{align*}
        f &\colon \prod_{y:\,A} (x\eq A y) \to A \\
...
\begin{lpar}{General path induction.}
    ...
    \[
        \Big(\sum_{x,y:\,A}x\eq A y\Big)\to\univ.
    \]
    ...
    \[
        f\colon\prod_{x,y:\,A}\;\prod_{p:\,x\eq Ay}C(x,y,p)
    \]
...
\begin{lpar}{The order relation on $\N$.}
    ...
    \[
        n\le m \defeq \sum_{k:\,\N} (n+k\eq\N m).
    \]
...
\begin{solution}
    ...
    \[
        \prod_{i,j:\,\N} i+j \eq\N j+i.
    \]
    We proceed by induction on $i$. Define the motive $C \colon \N \to \univ$ by
    \[
        C(i) \defeq \prod_{j:\,\N} i+j \eq\N j+i.
    \]
    
    \textbf{Base case:} We must provide a term $c_0 \colon \prod_{j:\,\N}0+j\eq\N j+0$.
    ...
    \[
        h \colon \prod_{j:\,\N} i+j \eq\N j+i.
    \]
