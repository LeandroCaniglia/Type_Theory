...
\begin{lpar}{Coproducts as Indexed Disjoint Unions.}\label{lpar:coproduct-as-disjoint-union}
    ...
    \begin{equation}\label{eq:inA-inB}
        ...
        \end{aligned}
    \end{equation}
    To see that these are well-defined specifications, note that for any $a\colon A$, the pair $(0_{\fun2},a)$ has type $\sum_{x:\,\fun2}\sigma(x)$ because the second component is of the required type $\sigma(0_{\fun2})\equiv A$. A similar argument holds for $\fun{in}_B$ since $\sigma(1_{\fun2}) \equiv B$.
...
\begin{lpar}{General path induction.}
    ...
    \begin{equation}\label{eq:ind=A}
        \fun{ind}_{\eq A}\colon\prod_{C:\,\prod_{x,y:\,A}(x\eq A y)\to\univ}
            \;\bigg(\prod_{x:\,A}C(x,x,\fun{refl}_x)\bigg)
...
\begin{lem}
    ...
    \begin{enumerate}[a), font=\upshape]
        \item $p^{-1}\cdot p\eq{y\eq Ay}\fun{refl}_y$ and\/ $p\cdot p^{-1}\eq{x\eq Ax}\fun{refl}_x$.
        \item $(p^{-1})^{-1}\eq{x\eq Ay}p$.
        \item $p\cdot(q\cdot r)\eq{x\eq Aw}(p\cdot q)\cdot r$, for\/ $q\colon y\eq Az$ and\/ $r\colon z\eq Aw$.
    \end{enumerate}
\end{lem}
...
\begin{lpar}{Substitution.}\label{lpar:substitution}
    The principle of \textsl{indiscernibility of identicals} establishes that the equality constructor is compatible with \textsl{type substitution} (a.k.a.~\textsl{type transport}).\footnote{See also Exercise~\ref{exr:ap-application}.}
...
\begin{lpar}{Based path induction.}
    ...
    \begin{equation}\label{eq:ind'=A}
        \fun{ind}'_{\eq A}\colon\prod_{a:\,A}\;\;
            \Bigg(
            \prod_{D:\,\prod_{x:\,A} (a\eq A x)\to\univ}D(a,\fun{refl}_a)
            \Bigg)
        \to\prod_{x:\,A}\;\prod_{p:\,a\eq Ax}D(x,p)
    \end{equation}
    with the equality
    \[
...
\begin{lpar}{Path induction equivalence.}
    ...
    \begin{equation}\label{eq:ind-domain-function}
        C(u,v,q\colon u\eq Av)\defeq\prod_{s:\,a\eq Au}D(u,s)\to D(v,s\ct q).
    \end{equation}
    For the base case  $c(u)$ we need a term of type
    \[
        C(u,u,\fun{refl}_u)\equiv\prod_{s:\,a\eq Au}D(u,s)\to D(u,s\ct\fun{refl}_u).
    \]
    ...
    Since we need to transform terms of $D(u,s)$ into terms of $D(u,s\ct\fun{refl}_u)$, we define $P_u\defeq\lambda s.\,D(u,s)$ and use the function $\fun{subst}_{P_u}$
    \begin{equation}\label{eq:ind-base-case-subst}
        c(u)\defeq\lambda s.\,\fun{subst}_{P_u}(\fun{unit}_r(s)^{-1})
...
\begin{lpar}{Successor injectivity.}
    ...
    \begin{proof}
        ...
        \[
            \prod_{n:\,\N}\fun{succ}(n)\eq\N0\to\fun0.
        \]
        ...
        \[
            \fun{subst}_P(p^{-1})\colon P(0)\to P(\fun{succ}(n)).
        \]
    ...
    \begin{proof}
        ...
        \[
            \prod_{n:\,\N}(\fun{succ}(n)\eq\N\fun{succ}(m)) \to (n\eq\N m).
        \]
