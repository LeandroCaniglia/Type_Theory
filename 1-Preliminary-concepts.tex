...
\begin{lpar}{General path induction.}
    In the case of identity types, the induction principle, a.k.a.~\textsl{path induction}, establishes that given a family 
    \[
        C\colon\prod_{x,y:A}(x\eq A y)\to\univ
    \]
    and a function $c\colon\prod_{x:A}C(x,x,\fun{refl}_x)$, there is a function
    \[
...
\begin{proof}
    ...
    \[
        C(x,y) \defeq \prod_{e:x\eq Ay}P(x)\to P(y).
    \]
...
\begin{exr}\label{exr:ap-application}
    ...
    \begin{enumerate}[a), font=\upshape]
        \item Define the \textsl{action on paths}, a.k.a.~\textsl{application}, function\/ $\fun{ap}_f$, which takes a function\/ $f\colon A\to B$ and a path\/ $p\colon x\eq Ay$ and produces a path\/ $f(x)\eq Bf(y)$.
