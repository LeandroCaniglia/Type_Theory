...
\begin{lpar}{Path induction equivalence.}
    ...
    \[
        f(x, p)\defeq E(a,x,p).
    \]
    This is well-typed because, according with \eqref{eq:ind-domain-function}, $E(a,x,p)$ provides a map $D(a,\fun{refl}_a)\to D(x,\fun{refl}_a\cdot p)$, and since $\fun{refl}_a\cdot p\equiv p$, the codomain is precisely $D(x,p)$.
    
    To verify the computation rule, we have to prove that
    \[
        f(a,\fun{refl}_a)\equiv d.
    \]
    But $f(a,\fun{refl}_a)\equiv c(a)(\fun{refl}_a)(d)$ and
    \begin{align*}
        c(a)(\fun{refl}_a)
