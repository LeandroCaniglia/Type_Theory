...
\section{Coproduct Types}
    ...
    \end{align*}
    There is also a nullary version, the \textsl{empty coproduct} (i.e., the coproduct of no type), denoted by $\fun0\colon\univ$, which has no constructors.
...
\begin{lpar}{Exactly two booleans.}
    ...
    \[
        C\colon\fun2\to\univ,\quad
            C\defeq\lambda x.\,(x=_{\fun2}0_{\fun2})+(x=_{\fun2}1_{\fun2}).
    \]
    Now,
    \begin{align*}
        C(0_{\fun2})&\equiv
            (0_{\fun2}=_{\fun2}0_{\fun2})
                +(0_{\fun2}=_{\fun2}1_{\fun2}),\\
        C(1_{\fun2})&\equiv
            (1_{\fun2}=_{\fun2}0_{\fun2})
                +(1_{\fun2}=_{\fun2}1_{\fun2}).
    \end{align*}
    Therefore,
    \[
    ...
    \[
        \fun{ind}_{\fun2}\big(C,
            ...
            \colon\prod_{x:\fun2}
                (x=_{\fun2}0_{\fun2})+(x=_{\fun2}1_{\fun2}).
    \]
\end{lpar}
...
\begin{lpar}{Coproducts as Indexed Disjoint Unions.}
    ...
    \end{align*}
    To verify the first equation, by the definition of $A\oplus B$, it suffices to consider the cases $p\defeq(0_{\fun2},a)$ $p\defeq(1_{\fun2},b)$, with $a\colon A$ and $b\colon B$.
    \begin{itemize}
...
\begin{lpar}{Multiplication of natural numbers.}
    ...
    \]
    we apply primitive recursion with the motive $C\defeq\N\to\N$, and specify:
    \begin{align*}
...
\begin{lpar}{Induction principle.}
    ...
    \]
    where $n$ is a generic natural, $h\colon C(n)$ is our \textsl{inductive hypothesis} and \textit{witness} is an element of $C(\fun{succ}(n))$.
\end{lpar}
...
\begin{lpar}{Predicate logic.}\label{lpar:predicate-logic}
    The following table extends the interpretation of propositions as types (see \nref{lpar:propositional-logic}) to quantifiers
    \[
        \begin{tabular}{l|ll}
            \hline
...
\begin{lpar}{The order relation on $\N$.}
    According to the translation table (see \nref{lpar:predicate-logic}), the relation $n\le m$, where $n,m\colon\N$, can be defined as
    \[
